...
function poolIsFourDistinctSingles(pool=[]){return pool.length===4&&new Set(pool.map(Number)).size===4}
function setControllerFromRolls(room){const ex=room.game.extra,ids=order(room),a=sum(ex.rolls[ids[0]]),b=sum(ex.rolls[ids[1]]);ex.pool=[...ex.rolls[ids[0]],...ex.rolls[ids[1]]];if(a===b){ex.phase='bigRoll';ex.bigRolls={[ids[0]]:null,[ids[1]]:null};ex.message=`Both totals are ${a}. Roll the large tiebreak die.`;return}ex.controllerId=a>b?ids[0]:ids[1];if(poolIsFourDistinctSingles(ex.pool)){ex.phase='allocateSingles';ex.message=`${P(room,ex.controllerId).name} controls the four dice (${Math.max(a,b)} to ${Math.min(a,b)}). Tap any two dice to keep; the other two go to ${P(room,opponent(room,ex.controllerId)).name}.`;return}ex.phase='allocate';ex.message=`${P(room,ex.controllerId).name} controls the four dice (${Math.max(a,b)} to ${Math.min(a,b)}). Choose how the matching dice are assigned.`}
function setControllerFromBig(room){const ex=room.game.extra,ids=order(room),a=ex.bigRolls[ids[0]],b=ex.bigRolls[ids[1]];if(a===b){ex.bigTieCount++;ex.bigRolls={[ids[0]]:null,[ids[1]]:null};ex.bigDieSide=ex.bigDieSide==='right'?'left':'right';ex.message=`Large die tied at ${a}. Roll it again.`;return}ex.controllerId=a>b?ids[0]:ids[1];ex.phase='allocateBig';const plans=blackAllocationPlans(room,{big:true});if(plans.length===1){beginMovement(room,plans[0].assignments);ex.message=`${P(room,ex.controllerId).name} wins the large die ${Math.max(a,b)} to ${Math.min(a,b)} and plays all four normal dice.`;return}ex.message=`${P(room,ex.controllerId).name} wins the large die ${Math.max(a,b)} to ${Math.min(a,b)}. Choose the matching-set direction.`}
...
function singleAllocationActions(room){const ex=room.game.extra,c=ex.controllerId,o=opponent(room,c),pool=[...ex.pool],out=[];if(!poolIsFourDistinctSingles(pool))return out;for(let i=0;i<pool.length;i++)for(let j=i+1;j<pool.length;j++){const own=[pool[i],pool[j]],rest=pool.filter((_,k)=>k!==i&&k!==j);out.push({action:'blackAllocateSingles',label:`${playerLabel(room,c)} keeps ${own.join(' + ')} · ${playerLabel(room,o)} gets ${rest.join(' + ')}`,args:{values:own}})}return out}
...
export function actionBlackGammon(room,p,{action,args={}}){const ex=room.game.extra,ids=order(room);if(action==='blackRoll'){if(ex.phase!=='rolling')throw new Error('Normal dice are not being rolled now');if(ex.rolls[p.id])throw new Error('You already rolled');ex.rolls[p.id]=[rollDie(),rollDie()];ex.message=`${p.name} rolled ${ex.rolls[p.id].join(' + ')}.${ids.every(id=>ex.rolls[id])?' Comparing totals…':' Waiting for the other roll.'}`;if(ids.every(id=>ex.rolls[id]))setControllerFromRolls(room);return}
 ...
 if(action==='blackAllocateSingles'){if(ex.phase!=='allocateSingles'||ex.controllerId!==p.id)throw new Error('You do not control this dice split');const values=(args.values||[]).map(Number);if(values.length!==2||new Set(values).size!==2)throw new Error('Choose exactly two different dice');const pool=[...ex.pool],used=[];for(const v of values){const i=pool.findIndex((x,idx)=>Number(x)===v&&!used.includes(idx));if(i<0)throw new Error('Choose dice from the shared roll');used.push(i)}const other=pool.filter((_,i)=>!used.includes(i)),o=opponent(room,p.id);beginMovement(room,{[p.id]:values.map(singleToken),[o]:other.map(singleToken)});return}
