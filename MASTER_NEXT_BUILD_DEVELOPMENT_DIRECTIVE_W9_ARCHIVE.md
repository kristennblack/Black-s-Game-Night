...
# PHASE W.9 LOCKED ADDENDUM - PROP HUNT CHARACTER DETAIL + CONTROL QUALITY

**Governing detail:** `MASTER_PHASE_W9_PROP_HUNT_CHARACTER_DETAIL_CONTROLS_DIRECTIVE.md`

Phase W.9 deepens W.5 approved-character identity and W.7 Prop Hunt combat presentation. It does not redesign any approved character. It defines how to add dimensional detail and how the control system must behave on phone, desktop and gamepad.

## W.9 character detail lock
- Approved turnarounds remain the absolute identity reference.
- Prop Hunt close-camera LOD0 may increase to roughly 8,000 to 12,000 triangles for approved hero humans, with lower LODs for medium/distant use.
- Spend detail on face/head silhouette, sculpted hair, shoulders/arms/hands, major clothing volume and shoes before tiny decorative geometry.
- Use soft stylized PBR with useful roughness/normal variation. Do not make skin, hair, denim and leather one flat material.
- Faces require rounded cheek/jaw/nose/lid/mouth construction sufficient for subtle expressions without changing the approved cartoon identity.
- Hair uses layered sculpted masses, not helmet blobs and not strand-heavy simulation.
- Backwards palms/hands are a hard release blocker. Negative-scale mirroring is not an acceptable runtime fix.
- Hunter uses right trigger hand, left support hand IK, explicit muzzle socket and stable two-hand aim pose.
- Clothing must have major 3D volume and clean deformation during run/jump/mantle rather than reading as painted cylinders.

## W.9 approved Prop Hunt production cards
Use the exact five-view turnaround as authority. Detailed written cues are defined in the governing W.9 file for:
- John;
- Kristen;
- Holly;
- Vanessa;
...
Papa, Nana, Kelsi, Molly and Gunner remain pending individual turnaround approval and must not be redesigned under this pass.

## W.9 control lock
- Camera-relative analog movement on all platforms.
- Touch/gamepad preserve partial stick magnitude with approximately 8 to 12 percent dead zone and gentle centre response.
- Run acceleration target roughly 0.20 to 0.30 s; prompt braking target roughly 0.15 to 0.22 s after release.
- Sprint remains approximately 125 to 140 percent of baseline run speed, subject to actual-device tuning.
- Preserve jump buffering, coyote time and variable jump height.
- Jump also drives validated automatic mantle. No separate climb button is required.
- Collision slides along walls and decorative `solid:false` geometry cannot trap camera/player.
- Hunter camera remains close right shoulder by default with shoulder swap and Reset View.
- Hider camera is slightly wider and recentres safely after disguise transformation.
...
- right open-view drag to look;
- large SHOOT;
- JUMP;
- nearby Sprint;
- smaller shoulder swap and Reset View.

No mandatory aim button. Touch/gamepad aim assist may use small-cone reticle friction/bias only. It may not rotate the camera for the player, see through walls or create target outlines through cover.

## W.9 hider controls
Existing locked rules remain: health carries across disguise changes, up to three disguise changes, one flash per disguise, ten total decoys, jump/run/climb on reasonable surfaces.

Hider UI must expose:
- CHANGE PROP with remaining change count;
- DECOY with remaining total;
- FLASH with current-disguise availability;
- JUMP;
- Sprint;
- shoulder swap/Reset View.

Transformation, decoy and flash actions must preserve movement/camera control immediately after the action and must fail safely if the target placement would intersect invalid geometry.

## W.9 shot-control contract
The camera crosshair, intended target ray, upper-body aim, muzzle obstruction validation, actual hit, tracer and impact effect must all describe the same shot. Every shot must visibly show muzzle feedback, a fast tracer and the real impact point.

## W.9 actual-device gate
John remains the first complete proof. Before propagation, actual-phone QA must demonstrate:
- approved John likeness with visibly improved dimensional face/hair/beard/clothing/hands;
- correct two-hand grip and no backwards hands;
- readable weapon and crosshair;
- responsive move/look/jump/shoot multi-touch;
- clean strafe, jump and mantle;
- visible tracer and impact;
- no camera collapse/top-down/pinned state;
- no mismatch between crosshair and hit.

Automated tests support this gate but cannot replace it.
