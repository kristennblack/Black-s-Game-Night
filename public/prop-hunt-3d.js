 ...
 * v2.0.0-prop-true3d-alpha rules/input layer with WebGL renderer
 *
...
(function(){
  ...
  let webgl=null,webglInitToken=0,webglFailed=false;
  ...
  function stop(){
    if(raf)cancelAnimationFrame(raf);raf=0;last=0;state=null;if(shootTimer)clearInterval(shootTimer);shootTimer=0;keysClear();webglInitToken++;if(webgl){try{webgl.dispose()}catch{}webgl=null;}ctx=null;
  ...
  function renderGame(){
    ...
    root.innerHTML=`<div class="game-title-row ph3-title-row"><div><span class="eyebrow">ROUND ${state.round} · DELUXE PROP HUNT</span><h1>Family Prop Hunt</h1><p class="subtext">${state.map.name} · ${state.map.ambient}</p></div><div class="button-row"><button id="ph3Restart" class="btn secondary">New match</button></div></div>
      <div class="ph3d-shell ph3d-shell-deluxe">
        <section class="ph3d-stage" id="ph3Stage">
          ...
          <canvas class="ph3d-minimap-overlay" id="ph3MiniMap" width="180" height="128" aria-label="Your map position"></canvas>
          ...
          <div class="ph3d-self-status" id="ph3SelfStatus"></div>
    ...
    canvas=root.querySelector('#ph3Canvas');ctx=null;webglFailed=false;
    root.querySelector('#ph3Restart').addEventListener('click',()=>{stop();renderSetup()});
    bindControls();initWebGL();resizeCanvas();window.addEventListener('resize',resizeCanvas);window.addEventListener('keydown',onKeyDown);window.addEventListener('keyup',onKeyUp);
    updateHud();last=performance.now();raf=requestAnimationFrame(loop);
  }
  function initWebGL(){
    const token=++webglInitToken;
    const startFallback=(err)=>{if(token!==webglInitToken||!canvas)return;console.warn('Prop Hunt WebGL unavailable; using legacy canvas renderer.',err||'');webglFailed=true;webgl=null;ctx=canvas.getContext('2d',{alpha:false});resizeCanvas();};
    if(!window.PropHuntWebGL?.create){startFallback('renderer module missing');return;}
    window.PropHuntWebGL.create(canvas,{getState:()=>state,propShape}).then(r=>{if(token!==webglInitToken||!state){try{r.dispose()}catch{}return;}webgl=r;ctx=null;webgl.rebuild(state);root?.querySelector('#ph3Stage')?.classList.add('webgl-active');resizeCanvas();const e=root?.querySelector('#ph3MoveStatus');if(e)e.innerHTML='<b>TRUE 3D WEBGL ACTIVE</b> · camera collision · volumetric props · 3D characters · 3D prop-zapper';}).catch(startFallback);
  }
  ...
  function resizeCanvas(){if(!canvas)return;if(webgl){webgl.resize();return;}if(!ctx)return;const r=canvas.getBoundingClientRect(),dpr=Math.min(2,devicePixelRatio||1);canvas.width=Math.max(320,Math.round(r.width*dpr));canvas.height=Math.max(360,Math.round(r.height*dpr));canvas._cssW=r.width;canvas._cssH=r.height;canvas._dpr=dpr;ctx.setTransform(dpr,0,0,dpr,0,0);}

  function loop(now){if(!state||!state.running)return;const dt=Math.min(.04,(now-last)/1000||.016);last=now;update(dt);if(webgl)webgl.render(state);else if(ctx)draw();raf=requestAnimationFrame(loop);}
  ...
  function shoot(){
    ...
    const cam=cameraData(),ray=webgl?.getShotRay?.()||{o:cam.cam,d:cam.forward};let hit=null,bestT=900;
  ...
  function drawMiniMapOverlay(){
    const c=root?.querySelector('#ph3MiniMap'),p=state?.player,m=state?.map;if(!c||!p||!m)return;const dpr=Math.min(2,devicePixelRatio||1),cssW=c.clientWidth||150,cssH=c.clientHeight||106;if(c.width!==Math.round(cssW*dpr)||c.height!==Math.round(cssH*dpr)){c.width=Math.round(cssW*dpr);c.height=Math.round(cssH*dpr);}const x=c.getContext('2d');x.setTransform(dpr,0,0,dpr,0,0);x.clearRect(0,0,cssW,cssH);x.fillStyle='rgba(11,9,7,.84)';x.fillRect(0,0,cssW,cssH);const pad=7,sx=(cssW-pad*2)/m.w,sz=(cssH-pad*2)/m.d,active=zoneForPoint(p.x,p.z);for(const z of m.zones||[]){x.fillStyle=z===active?'rgba(216,174,94,.28)':'rgba(143,116,75,.13)';x.strokeStyle=z===active?'rgba(255,220,143,.72)':'rgba(180,151,105,.28)';x.lineWidth=z===active?1.5:1;x.fillRect(pad+z.x*sx,pad+z.z*sz,z.w*sx,z.d*sz);x.strokeRect(pad+z.x*sx,pad+z.z*sz,z.w*sx,z.d*sz);}const px=pad+p.x*sx,pz=pad+p.z*sz;x.save();x.translate(px,pz);x.rotate(-p.yaw);x.fillStyle='#6ed9ff';x.strokeStyle='#082331';x.lineWidth=2;x.beginPath();x.moveTo(0,-7);x.lineTo(5,6);x.lineTo(0,3);x.lineTo(-5,6);x.closePath();x.fill();x.stroke();x.restore();x.fillStyle='#f8e5b7';x.font='800 7px system-ui';x.fillText('YOU',Math.min(cssW-22,px+7),Math.max(9,pz-6));}
  ...
  function updateHud(){
    ...
    drawMiniMapOverlay();
    const selfStatus=root.querySelector('#ph3SelfStatus');if(selfStatus)selfStatus.innerHTML=isHider?`<b>YOU · ${p.person.name}</b><span>${p.prop?`DISGUISED AS ${p.prop.toUpperCase()}`:'VISIBLE FAMILY CHARACTER'}</span><small>${p.prop?(p.locked?'LOCKED & STILL':'MOBILE DISGUISE'):'Move beside a real object and tap PROP'}</small>`:`<b>YOU · ${p.person.name} · HUNTER</b><span>3D PROP-ZAPPER · ${p.reload>0?'RELOADING':p.ammo+'/30'}</span><small>${hunterWaiting?'Waiting for hiders…':'Aim at suspicious props and SHOOT'}</small>`;
