 ...
 * v3.10.0-phase-w7-character-combat-vertical-slice
 *
...
(function(){
  ...
  function startEngine(mapKey){
    ...
    game={renderer,scene,camera,clock,mapKey,world:null,actors:[],actorsById:new Map(),player:null,keys,feed:[],effects:[],decoys:[],cameraYaw:Math.PI,cameraPitch:.18,cameraDistance:4.35,cameraActualDistance:4.35,shotCooldown:0,recoil:0,round:roomState.round||1,lastNetworkSend:0,startedAt:performance.now(),nearProp:null,controlDisposers:[],padPrev:{},padShoot:false,padSprint:false,spectateTarget:null,ghostMode:'free',ghost:null,lastSummaryRound:0,lastDamageAt:0,lastPhase:roomState.phase,hideReleaseAnnounced:false,roundSeed:roomState.roundSeed||0,layoutVariant:roomState.layoutVariant||0,weatherPreset:roomState.weatherPreset||'clear',qa:{frames:0,accum:0,fps:0,lastHud:0}};
  ...
  function revalidateShotFromMuzzle(a,cameraPoint,targets){const start=muzzleWorldPosition(a),dir=cameraPoint.clone().sub(start),distance=Math.min(32,dir.length()+.18);if(distance<.02)return{hitPoint:cameraPoint,target:null,hit:null};dir.normalize();const muzzleRay=new THREE.Raycaster(start,dir,.01,distance),hits=muzzleRay.intersectObjects(targets,false).filter(h=>h.object.visible);if(!hits.length)return{hitPoint:cameraPoint,target:null,hit:null};return{hitPoint:hits[0].point.clone(),target:actorFromRayHit(hits[0]),hit:hits[0]}}
  ...
  function registerHit(shooter,target){if(roomState.phase!=='hunt'||!target.alive)return;if(network&&ws?.readyState===1){ws.send(JSON.stringify({type:'action',action:'hit',targetId:target.id}));return}target.health--;target.locked=false;target._hitShake=.22;if(target===game.player)showDamage(target.health);gameplay.playTransientAnimation(target,'hit',460);spawnSparks(new THREE.Vector3(target.x,target.y+target.height*.55,target.z),9);addFeed(`Hit ${target.prop||target.person.name}: ${Math.max(0,target.health)}/3.`);if(target.health<=0){spawnPropBreak(target);playSinCue();if(shooter){shooter.score=shooter.score||{hunterElims:0};shooter.score.hunterElims=(shooter.score.hunterElims||0)+1}if(roomState.settings.mode==='chaos'){target.role='hunter';target.health=3;target.prop=null;target.alive=true;target.disguiseOptions=[];applyActorVisual(target);addFeed(`${target.person.name} joins the hunters.`)}else{target.alive=false;target.rig.visible=false;beginGhostMode(target);addFeed(`${target.person.name} was found.`)}}}
  function spawnTracer(a,end){
    const parts=a.rig.userData.parts,muzzle=parts?.weapon?.userData?.muzzle,start=new THREE.Vector3(a.x,a.y+a.height*.68,a.z);if(muzzle)muzzle.getWorldPosition(start);const dir=end.clone().sub(start),len=dir.length();if(len>.02){const mat=new THREE.MeshBasicMaterial({color:0x8ff4ff,transparent:true,opacity:.92,depthWrite:false}),beam=new THREE.Mesh(new THREE.CylinderGeometry(.014,.022,len,8,1,true),mat);beam.position.copy(start).add(end).multiplyScalar(.5);beam.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),dir.normalize());game.scene.add(beam);game.effects.push({kind:'tracer',mesh:beam,life:.15,max:.15})}
    const orb=new THREE.Mesh(new THREE.SphereGeometry(.055,10,7),new THREE.MeshBasicMaterial({color:0xc8fbff,transparent:true,opacity:1,depthWrite:false}));orb.position.copy(start);game.scene.add(orb);game.effects.push({kind:'tracer',mesh:orb,life:.07,max:.07});const flash=new THREE.PointLight(0x7eefff,4.2,2.8,2);flash.position.copy(start);game.scene.add(flash);game.effects.push({kind:'light',mesh:flash,life:.055,max:.055});
  }
  function spawnImpactBurst(pos,{actorHit=false,object=null}={}){const color=actorHit?0x8ff4ff:(object?.material?.metalness||0)>.45?0xffe19b:0xffc86a,mat=new THREE.MeshBasicMaterial({color,transparent:true,opacity:.9,depthWrite:false,side:THREE.DoubleSide}),ring=new THREE.Mesh(new THREE.RingGeometry(.055,.11,20),mat);ring.position.copy(pos);ring.quaternion.copy(game.camera.quaternion);game.scene.add(ring);game.effects.push({kind:'ring',mesh:ring,life:.24,max:.24,grow:2.2});const light=new THREE.PointLight(color,actorHit?3.2:2.1,1.7,2);light.position.copy(pos);game.scene.add(light);game.effects.push({kind:'light',mesh:light,life:.08,max:.08});}
  ...
  window.__PROP_HUNT_REAL3D__={version:'GAME-NIGHT-STAGING-PHASE-W7-PROP-HUNT-CHARACTER-COMBAT-32',renderer:'WebGL',three:'0.185.1',usesDepthBuffer:true,usesCanvas2D:false,p3DirectionalLocomotion:true,p3SpectatorCamera:true,p3PropTransform:true,t1HunterBlindHide:true,t1CrosshairFire:true,t1HoldRapidFire:true,phaseVWorldExpansion:true,phaseVMapScale:8.31,phaseVRotatingDisguises:true,phaseVWeather:true,phaseVInteractions:true,phaseVGhostMvp:true,w7ApprovedJohnFallback:true,w7UnapprovedLegacyModelBlocked:true,w7TwoHandWeaponGrip:true,w7HunterAimCamera:true,w7VisibleBeamTracer:true,w7ImpactBurst:true,worldRelease:'GAME-NIGHT-STAGING-PHASE-V-PROP-HUNT-WORLD-24'};
