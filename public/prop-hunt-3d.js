 ...
 * v3.1.0-p3-gameplay-animation-feel
 *
...
(function(){
  ...
  function startSolo(){
    ...
    for(let i=1;i<setup.count;i++){const p=pool[(i-1)%pool.length];players.push({id:`bot-${i}`,name:p.name,avatar:p.id,isBot:true,difficulty:'easy',ready:true,role:null,health:3,alive:true})}
  ...
  function startEngine(mapKey){
    ...
    game={renderer,scene,camera,clock,mapKey,world:null,actors:[],actorsById:new Map(),player:null,keys,feed:[],effects:[],decoys:[],cameraYaw:Math.PI,cameraPitch:.18,cameraDistance:3.65,cameraActualDistance:3.65,shotCooldown:0,recoil:0,round:roomState.round||1,lastNetworkSend:0,startedAt:performance.now(),nearProp:null,controlDisposers:[],padPrev:{},padAim:false,padSprint:false,spectateTarget:null,lastDamageAt:0,qa:{frames:0,accum:0,fps:0,lastHud:0}};
  ...
  function gameShell(){return `<div class="ph3d-shell"><section id="ph3Stage" class="ph3d-stage"><canvas id="ph3Canvas" class="ph3d-canvas"></canvas>${QA_MODE?'<pre id=\"ph3Qa\" class=\"ph3d-qa\"></pre>':''}<div class="ph3d-top"><span id="ph3Role" class="ph3d-chip role"></span><span id="ph3Phase" class="ph3d-chip map"></span><span id="ph3Health" class="ph3d-chip health"></span></div><div id="ph3Crosshair" class="ph3d-crosshair"></div><button id="phShoulder" class="ph3d-shoulder no-look" aria-label="Swap camera shoulder" title="Swap camera shoulder">↔</button><button id="phResetView" class="ph3d-reset-view no-look" aria-label="Reset camera and recover player" title="Reset camera and recover player">↺</button><button id="phSpectate" class="ph3d-spectate no-look" hidden>NEXT</button><div id="ph3Hit" class="ph3d-hit">x</div><div id="ph3Flash" class="ph3d-flash"></div><div id="ph3Damage" class="ph3d-damage"></div><div id="ph3Prompt" class="ph3d-prop-prompt"></div>${QA_MODE?'<div class="ph3d-camera-help">QA controls: drag look · left stick/WASD move · pinch/wheel zoom · R resets view.</div>':''}<div class="ph3d-controls"><div id="phJoy" class="ph3d-joystick"><div id="phStick" class="ph3d-stick"></div></div><div class="ph3d-actions"><button id="phAim" class="ph3d-act aim">AIM</button><button id="phShoot" class="ph3d-act primary">SHOOT</button><button id="phJump" class="ph3d-act jump">JUMP</button><button id="phSprint" class="ph3d-act sprint">SPRINT</button><button id="phProp" class="ph3d-act prop">PROP</button><button id="phFlashBtn" class="ph3d-act flash">FLASH</button><button id="phDecoy" class="ph3d-act">DECOY</button><button id="phLock" class="ph3d-act lock">LOCK</button></div></div></section><aside class="ph3d-side"><div class="ph3d-mini"><h3>Loadout</h3><div id="ph3Load" class="ph3d-readout"></div></div><div class="ph3d-mini"><h3>Quick controls</h3><div class="ph3d-legend"><span>Move & run</span><span>Aim & shoot</span><span>Jump & climb</span><span>Change prop</span></div></div><div class="ph3d-mini"><h3>Family feed</h3><div id="ph3Feed" class="ph3d-feed"></div></div></aside></div>`;}
  ...
  function baseLighting(scene,night=false){const hemi=new THREE.HemisphereLight(night?0x7b89a7:0xd1e2ec,night?0x302f30:0x5d5547,night?1.0:1.28);scene.add(hemi);const ambient=new THREE.AmbientLight(night?0x8996b2:0xfff1dc,night?.18:.19);scene.add(ambient);const sun=new THREE.DirectionalLight(night?0xffd2aa:0xffedc9,night?2.0:2.75);sun.position.set(-11,18,10);gameplay.configureShadowCastingLight(sun,{mapSize:2048,left:-28,right:28,top:28,bottom:-28,near:.5,far:55});scene.add(sun);const fill=new THREE.DirectionalLight(night?0x7186ae:0x9cc5db,night?.18:.34);fill.position.set(12,9,-10);scene.add(fill);}
  ...
  async function upgradePapaProductionSlice(w){
    ...
    // Keep the accepted gameplay AABBs/colliders but stop drawing their prototype surfaces.
  ...
  function updatePlayer(dt){
    const a=game.player;if(!a.alive){a.vx=a.vz=0;return}if(a.locked&&a.prop){a.vx=a.vz=0;animateMantle(a,dt);return}if(a.mantle){animateMantle(a,dt);return}
    const aiming=input.aim||game.padAim,intent=gameplay.movementIntent(keys,joy,game.cameraYaw),sprint=gameplay.wantsSprint(keys,input,{sprint:game.padSprint},intent);
    ...
    const moving=Math.hypot(a.vx,a.vz)>.15;a._moving=moving;a._directional=gameplay.resolveDirectionalLocomotion(a,{aiming,sprinting:sprint});
    if(moving&&!a._wasMoving){a._moveTransition='startMove';a._moveTransitionUntil=performance.now()+240}else if(!moving&&a._wasMoving){a._moveTransition='stopMove';a._moveTransitionUntil=performance.now()+280}
    moveActor(a,a.vx*dt,a.vz*dt,(a._jumpBuffer||0)>0||a.vy>1.2);resolveActorOverlap(a);input.jumpQueued=false;
    ...
    const support=core.supportHeight(a.x,a.z,a.radius,game.world.colliders,a.y+.08,.46);if(a.y<=support){const impact=Math.max(0,-a.vy);if(impact>.8){a._landingStrength=clamp((impact-2.5)/7.5,.12,1);a.landTimer=impact>8.1?.24:.14;a._hardLandTimer=impact>8.1?.22:0}a.y=support;a.vy=0;a.grounded=true}else a.grounded=false;
    a.landTimer=Math.max(0,(a.landTimer||0)-dt);a._hardLandTimer=Math.max(0,(a._hardLandTimer||0)-dt);a._landingStrength=Math.max(0,(a._landingStrength||0)-dt*3.7);a.anim=gameplay.resolveLocomotionAnim(a,{moving,sprinting:sprint,aiming});
  ...
  function handleNetworkAction(m){if(m.action==='flash'){const source=game.actorsById.get(m.playerId);if(source&&game.player.role==='hunter'&&core.dist2(source,game.player)<3.5)flashScreen();addFeed(`${source?.person?.name||'A hider'} fired a flash.`)}if(m.action==='hit'){const target=game.actorsById.get(m.targetId);if(target){target.health=m.health;target.alive=m.alive;target.role=m.role;if(target===game.player)showDamage(target.health);gameplay.playTransientAnimation(target,'hit',460);if(!target.alive)spawnPoof(target.x,target.y+.6,target.z);addFeed(`${target.person.name} was hit. ${target.health}/3.`)}}if(m.action==='decoy')spawnDecoy(m.prop,m.position?.x||0,m.position?.y||0,m.position?.z||0);}
  ...
  function updateActorVisuals(dt){
    for(const a of game.actors){
      ...
      a.recoil=Math.max(0,(a.recoil||0)-dt*7);a.rig.visible=!(a===game.player&&a.cameraHidden);const speed=Math.hypot(a.vx||0,a.vz||0),aiming=a.role==='hunter'&&(a===game.player?(input.aim||game.padAim):!!a.ai?.detected),directional=gameplay.resolveDirectionalLocomotion(a,{aiming,sprinting:!!a._sprinting});a._directional=directional;
      let focus=null;
  ...
  function updateEffects(dt){for(let i=game.effects.length-1;i>=0;i--){const e=game.effects[i];e.life-=dt;if(e.kind==='tracer')e.mesh.material.opacity=Math.max(0,e.life/e.max);if(e.kind==='ring'){const k=1+(1-e.life/e.max)*(e.grow||1);e.mesh.scale.setScalar(k);if(e.mesh.material)e.mesh.material.opacity=Math.max(0,e.life/e.max)}if(e.kind==='spark'){e.vy-=8*dt;e.mesh.position.x+=e.vx*dt;e.mesh.position.y+=e.vy*dt;e.mesh.position.z+=e.vz*dt;e.mesh.material.opacity=Math.max(0,e.life/e.max)}if(e.life<=0){game.scene.remove(e.mesh);e.mesh.geometry?.dispose?.();e.mesh.material?.dispose?.();game.effects.splice(i,1)}}game.motionFx?.update(dt);if(game.world?.group)art.animateAmbience(game.world.group,performance.now()*.001,{player:game.player,dt})}

  function updateCamera(dt){
    const player=game.player;if(!player)return;let a=player;if(!player.alive&&roomState.settings.mode==='classic'){if(!game.spectateTarget?.alive)game.spectateTarget=game.actors.find(o=>o.alive&&o!==player)||null;if(game.spectateTarget)a=game.spectateTarget}
    ...
    const aim=a===player&&(input.aim||game.padAim),sprinting=a===player&&gameplay.wantsSprint(keys,input,{sprint:game.padSprint},{strength:Math.min(1,Math.hypot(a.vx,a.vz)/(game.cameraRig.cfg.runSpeed||1))});game.cameraRig.state.aim=aim;game.cameraRig.update(a,game.world.colliders,dt,{aim,sprinting,dog:isDog(a.person),height:a.prop?a.height*.55:(isDog(a.person)?.64:1.17),velocity:{x:a.vx,z:a.vz},turnRate:a.turnRate||0,cameraBob:(a._motion?.landing||0)*-.032});
    game.cameraYaw=game.cameraRig.state.yaw;game.cameraPitch=game.cameraRig.state.pitch;game.cameraActualDistance=game.cameraRig.state.actualDistance;player.cameraHidden=player.alive&&game.cameraActualDistance<.72&&!player.prop;updateAimAssistIndicator();
  }
  ...
  function updateAimAssistIndicator(){const e=root.querySelector('#ph3Crosshair');if(!e||!game?.player)return;const target=(input.aim||game.padAim)?aimAssistTarget(game.player):null;game.aimAssist=target;e.classList.toggle('assisted',!!target);e.setAttribute('data-assist',target?'target':'none')}
  function shoot(){
    const a=game?.player;if(!a||a.role!=='hunter'||!a.alive||roomState.phase!=='hunt'||game.shotCooldown>0)return;
    game.shotCooldown=.11;a.recoil=.7;gameplay.playTransientAnimation(a,'fire',240);audio?.oneShot('zap',{volume:.12,pitch:.92+Math.random()*.12});game.cameraRig?.kick(.028,(Math.random()-.5)*.008,.025);
    const ray=new THREE.Raycaster();ray.setFromCamera(new THREE.Vector2(0,0),game.camera);ray.far=28;const targets=[...game.world.raycastMeshes];for(const d of game.decoys)d.traverse(o=>{if(o.isMesh&&o.visible)targets.push(o)});for(const n of game.world.npcs||[])n.traverse(o=>{if(o.isMesh&&o.visible)targets.push(o)});for(const b of game.actors)if(b!==a&&b.alive)b.rig.traverse(o=>{if(o.isMesh&&o.visible)targets.push(o)});
  ...
  function cycleSpectate(){if(!game||game.player?.alive)return;const alive=game.actors.filter(a=>a.alive&&a!==game.player);if(!alive.length)return;const i=Math.max(-1,alive.indexOf(game.spectateTarget));game.spectateTarget=alive[(i+1)%alive.length];game.cameraRig.reset(game.spectateTarget,game.world.colliders,{yaw:game.spectateTarget.yaw,pitch:.065,distance:game.cameraRig.cfg.cameraDistance,forceSnap:true,reason:'spectator target change'});}
  ...
  function applyDisguise(a,type,announce=true){
    if(a.prop&&a.propChanges<=0)return;audio?.oneShot('ui',{volume:.07,pitch:.72});spawnTransformBurst(a.x,a.y+a.height*.42,a.z);if(a.prop)a.propChanges--;a.prop=type;a.flash=true;a.locked=false;applyActorVisual(a);a._propTransform=1;if(a.propMesh){a.propMesh.userData.p3BaseScale={x:a.propMesh.scale.x,y:a.propMesh.scale.y,z:a.propMesh.scale.z};a.propMesh.scale.multiplyScalar(.22)}if(announce)addFeed(`${a.person.name} disguised as ${type}. Health stayed at ${a.health}/3.`);
  }
  ...
  function spawnDecoy(type,x,y,z,{rotation=null}={}){const mesh=createPropMesh(type);mesh.position.set(x,Number.isFinite(Number(y))?Number(y):0,z);mesh.rotation.y=Number.isFinite(rotation)?rotation:Math.random()*Math.PI*2;game.scene.add(mesh);game.decoys.push(mesh);spawnPlacementRing(mesh.position.x,mesh.position.y+.05,mesh.position.z);}
  ...
  function updateHud(){
    const a=game.player,phase=root.querySelector('#ph3Phase'),role=root.querySelector('#ph3Role'),health=root.querySelector('#ph3Health'),load=root.querySelector('#ph3Load'),feed=root.querySelector('#ph3Feed'),prompt=root.querySelector('#ph3Prompt'),spectate=root.querySelector('#phSpectate');if(!phase)return;const left=roomState.phaseEndsAt?Math.max(0,roomState.phaseEndsAt-Date.now()):0;
    phase.textContent=`${String(roomState.phase).toUpperCase()} ${left?fmt(left/1000):''} - ${mapName(game.mapKey)} - R${roomState.round}/${roomState.settings.rounds}`;const spectating=!a.alive&&roomState.settings.mode==='classic';role.textContent=spectating?`SPECTATING · ${game.spectateTarget?.person?.name||'family'}`:a.role==='hunter'?'HUNTER':`HIDER${a.prop?` · ${a.prop}`:''}`;const hp=Math.max(0,a.health);health.textContent=`HP ${'●'.repeat(hp)}${'○'.repeat(Math.max(0,3-hp))}`;
    load.innerHTML=a.role==='hider'?`Disguise: <b>${esc(a.prop||'none')}</b><br>Changes left: <b>${a.propChanges}</b><br>Decoys: <b>${a.decoys}/10</b><br>Flash: <b>${a.flash?'READY':'USED'}</b><br>Lock: <b>${a.locked?'LOCKED':'FREE'}</b><br>Health carries through disguise changes.`:`Weapon: <b>Prop Zapper</b><br>Aim: <b>${(input.aim||game.padAim)?'READY':'HIP'}</b><br>Shots stop at walls, props and players.<br>Unlimited reserve ammunition.`;
    if(feed){feed.innerHTML=game.feed.slice(-12).map(x=>`<div>${esc(x)}</div>`).join('');feed.scrollTop=feed.scrollHeight}game.nearProp=a.role==='hider'&&a.alive?nearestProp(a,1.65):null;if(prompt){if(game.nearProp){prompt.textContent=`PROP: ${game.nearProp.type}`;prompt.classList.add('on')}else prompt.classList.remove('on')}
    const propBtn=root.querySelector('#phProp'),flashBtn=root.querySelector('#phFlashBtn'),decoyBtn=root.querySelector('#phDecoy'),lockBtn=root.querySelector('#phLock');if(propBtn)propBtn.textContent=`PROP ${a.propChanges}`;if(flashBtn)flashBtn.textContent=a.flash?'FLASH ✓':'FLASH ×';if(decoyBtn)decoyBtn.textContent=`DECOY ${a.decoys}`;if(lockBtn)lockBtn.textContent=a.locked?'UNLOCK':'LOCK';
    for(const id of ['phProp','phFlashBtn','phDecoy','phLock']){const b=root.querySelector('#'+id);if(b){b.disabled=a.role!=='hider'||!a.alive;b.classList.toggle('role-hidden',a.role!=='hider'||spectating)}}for(const id of ['phAim','phShoot']){const b=root.querySelector('#'+id);if(b)b.classList.toggle('role-hidden',a.role!=='hunter'||spectating)}const shootBtn=root.querySelector('#phShoot');if(shootBtn)shootBtn.disabled=a.role!=='hunter'||roomState.phase!=='hunt'||!a.alive;if(spectate){spectate.hidden=!spectating;spectate.textContent=game.spectateTarget?`NEXT · ${game.spectateTarget.person.name}`:'NEXT';}
  }
  ...
  window.__PROP_HUNT_REAL3D__={version:'GAME-NIGHT-STAGING-PHASE-T-PROP-HUNT-P3-GAMEPLAY-ANIMATION-18',renderer:'WebGL',three:'0.185.1',usesDepthBuffer:true,usesCanvas2D:false,p3DirectionalLocomotion:true,p3SpectatorCamera:true,p3PropTransform:true};
