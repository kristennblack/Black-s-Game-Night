 ...
 * v4.4.0-phase-w40-external-asset-pipeline
 *
...
(function(){
  ...
  const W36_VISUAL_URL='/w36-leapfrog-visuals.mjs?v=W40-EXTERNAL-ASSET-PIPELINE-59';
  const W40_PRESENTATION_URL='/w40-production-presentation.mjs?v=W40-EXTERNAL-ASSET-PIPELINE-59';
  const W40_TELEMETRY_URL='/w40-runtime-telemetry.mjs?v=W40-EXTERNAL-ASSET-PIPELINE-59';
  const GAMEPLAY_URL='/shared-3d-gameplay.mjs?v=W40-EXTERNAL-ASSET-PIPELINE-59';
  ...
  const W40_TRUTH_OVERLAY=QA_MODE||URL_FLAGS.get('w40Telemetry')==='1'||URL_FLAGS.get('w40Truth')==='1';
  const CDN_NOTICE='Three.js 0.185.1';

  let root=null,THREE=null,core=null,art=null,gameplay=null,studio=null,w35vis=null,w36vis=null,w40pres=null,w40telemetry=null,phaseE=null,approvedCharacters=null,catalogData=null,worldCatalogData=null,assets=null,audio=null,loadPromise=null,game=null,raf=0,lastFrame=0;
  ...
  async function ensureEngine(){
    ...
    loadPromise=Promise.all([import(THREE_URL),import(CORE_URL),import(ART_URL),import(GAMEPLAY_URL),import(STUDIO_URL),import(W35_VISUAL_URL),import(W36_VISUAL_URL),import(W40_PRESENTATION_URL),import(W40_TELEMETRY_URL),import(PHASE_E_URL),import(APPROVED_CHAR_URL),import(CATALOG_URL),import(WORLD_CATALOG_URL)]).then(([t,c,a,g,s,v,v36,v40,t40,q,ch,cat,wcat])=>{THREE=t;core=c;art=a.create3DArtKit(THREE);gameplay=g;studio=s;w35vis=v;w36vis=v36;w40pres=v40;w40telemetry=t40;phaseE=q;approvedCharacters=ch;catalogData=cat;worldCatalogData=wcat;for(const id of ch.APPROVED_FAMILY_CHARACTER_IDS||[]){if(OUTFITS[id])Object.assign(OUTFITS[id],ch.approvedFallbackStyle(id,OUTFITS[id]));}assets=studio.createAuthoredAssetPipeline(THREE,{assetVersion:w40pres.W40_BUILD_ID||phaseE.STAGING_BUILD_ID});audio=studio.createAudioSystem();return true}).catch(err=>{console.error('3D engine failed to load',err);throw new Error('The 3D engine could not load. Check the internet connection and reload the game.')});
    return loadPromise;
  }

  function stop(){
    if(raf)cancelAnimationFrame(raf);raf=0;lastFrame=0;
    if(game?.w40Truth)game.w40Truth.dispose?.();
  ...
  function startEngine(mapKey){
    ...
    const camera=new THREE.PerspectiveCamera(w40pres?.W40_PROP_HUNT_CAMERA?.fov||56,1,.07,180);const clock=new THREE.Clock();
    game={renderer,scene,camera,clock,mapKey,world:null,actors:[],actorsById:new Map(),player:null,keys,feed:[],effects:[],decoys:[],cameraYaw:Math.PI,cameraPitch:(w40pres?.W40_PROP_HUNT_CAMERA?.pitch??.012),cameraDistance:(w40pres?.W40_PROP_HUNT_CAMERA?.cameraDistance??3.72),cameraActualDistance:(w40pres?.W40_PROP_HUNT_CAMERA?.cameraDistance??3.72),shotCooldown:0,recoil:0,round:roomState.round||1,lastNetworkSend:0,startedAt:performance.now(),nearProp:null,controlDisposers:[],padPrev:{},padShoot:false,padAim:false,padSprint:false,padJumpHeld:false,spectateTarget:null,ghostMode:'free',ghost:null,lastSummaryRound:0,lastDamageAt:0,lastPhase:roomState.phase,hideReleaseAnnounced:false,roundSeed:roomState.roundSeed||0,layoutVariant:roomState.layoutVariant||0,weatherPreset:roomState.weatherPreset||'clear',fxBudget:84,qualityTier:'high',frameAlpha:1,contextLost:false,stability:{recoveries:0,fixedSteps:0,droppedSimulationTime:0,lastReason:'none'},qa:{frames:0,accum:0,fps:0,lastHud:0,frameTimes:[],p95Ms:0,maxMs:0}};
    ...
    game.cameraRig=gameplay.createThirdPersonCamera(THREE,camera,core,'propHunt',{yaw:Math.PI,...(w40pres?.W40_PROP_HUNT_CAMERA||{})});
    w40pres?.applyProfessionalRenderer?.(renderer,camera);
    game.w40Truth=w40telemetry?.makeRuntimeTelemetry?.({root,getGame:()=>game,enabled:W40_TRUTH_OVERLAY})||null;
    game.w40EnvironmentLighting={state:'loading'};w40pres?.installWorkshopEnvironment?.(THREE,renderer,scene).then(status=>{if(game){game.w40EnvironmentLighting=status;game.world&&(game.world.w40EnvironmentLighting=status);game.w40Truth?.update?.(true)}}).catch(()=>{});
    game.performance=gameplay.createPerformanceGovernor(renderer,{targetFps:52,minPixelRatio:.78,maxPixelRatio:Math.min(2,devicePixelRatio||1),onQualityChange:tier=>applyPerformanceQuality(tier)});
    initEffectPool();
  ...
  function disposeRoot(){if(game?.w40Truth)game.w40Truth.dispose?.();if(game?.controlDisposers)for(const off of game.controlDisposers){try{off?.()}catch{}}try{game?.motionFx?.dispose?.()}catch{}try{disposeEffectPool()}catch{}if(game?.renderer){try{game.renderer.dispose()}catch{}}game=null;}
  ...
  async function upgradePapaProductionSlice(w){
    ...
    if(w.phaseVExpanded){
      ...
      const w40status=await w40pres?.tryPromoteExternalPapa?.({THREE,world:w,scene:game.scene,onStatus:s=>console.info('W40 external asset slot',s)});
      if(w40status){w.w40ExternalStatus=w40status;addFeed('W40 external-asset pipeline armed: incoming GLBs promote only after their manifest QA gate is enabled; fallbacks stay visible otherwise.')}
      addFeed("W36 Leapfrog world preserved under W40: full shop, barn, pens and yard remain visible while production replacements pass the gate.");
      return;
    }
  ...
  function buildHumanRigBase(style,opts){return art.buildHumanRig(style,opts);}
  function buildHumanRig(person,role){
    const style=OUTFITS[person.id]||OUTFITS.john,profile=bodyProfile(person.id),rig=buildHumanRigBase(style,{id:person.id,role,scale:profile.scale,proportions:profile.proportions});
    approvedCharacters?.tagWithApprovedIdentity?.(rig,person.id,{modelMatchesReference:false});
    return rig;
  }
  ...
  async function tryUpgradeAuthoredActor(actor){
    try{
      await assets.ensureManifest();
      const dog=isDog(actor.person),entry=assets.entry(dog?'dogs':'characters',actor.person.id),approvedSpec=!dog?approvedCharacters?.getApprovedFamilyCharacter?.(actor.person.id):null;
      ...
      if(weapon){
        ...
        studio.attachToRigSocket(rig,weapon,{socket,position:productionSocket?[0,0,0]:(dog?[0,.08,.04]:[0,-.02,.12]),rotation:productionSocket?[0,0,0]:(dog?[0,0,0]:[-Math.PI/2,0,0]),scale:dog?.34:.55});
  ...
  function resetPlayableView({announce=true}={}){const a=game?.player;if(!a)return;const moved=gameplay.recoverActorFromGeometry(core,a,game.world.colliders,game.world.bounds,{radius:a.radius,height:a.height,maxRadius:5,requireCameraPocket:true,cameraHeight:isDog(a.person)?.64:(game.cameraRig?.cfg?.cameraHeight||1.34),cameraDistance:game.cameraRig?.cfg?.cameraDistance||3.72,minCameraPocket:1.5});game.cameraRig.reset(a,game.world.colliders,{yaw:a.yaw,pitch:W36_BENCHMARK?(game._w36BenchmarkPitch??-.08):(w40pres?.W40_PROP_HUNT_CAMERA?.recoveryPitch??.012),distance:game.cameraRig.cfg.cameraDistance,dog:isDog(a.person),height:a.prop?a.height*.55:(isDog(a.person)?.64:(game.cameraRig?.cfg?.cameraHeight||1.34)),forceSnap:true,reason:'manual Reset View'});game.stagingQa?.setRecovery(moved?'manual Reset View + safe-position recovery':'manual Reset View');game.cameraYaw=game.cameraRig.state.yaw;game.cameraPitch=game.cameraRig.state.pitch;if(announce)addFeed(moved?'Player and view recovered to a safe spot.':'View reset behind your character.');}

  function loop(now){
    if(!game)return;
    const rawFrameDt=Math.max(.001,(now-lastFrame)/1000);lastFrame=now;
    if(game.contextLost){game.fixedStep?.reset?.();raf=requestAnimationFrame(loop);return}
    const frameDt=Math.min(.12,rawFrameDt);
    pollFrameInput(frameDt);
    const fixed=game.fixedStep.advance(frameDt,(dt)=>simulateFixed(dt,now));game.frameAlpha=fixed.alpha;game.stability.fixedSteps+=fixed.steps;game.stability.droppedSimulationTime=fixed.droppedTime;
    presentFrame(frameDt,now,fixed.alpha);game.performance?.sample(rawFrameDt);if(QA_MODE)updateQaHud(rawFrameDt,now);game.stagingQa?.update(frameDt,now);game.renderer.render(game.scene,game.camera);raf=requestAnimationFrame(loop);
  }
  ...
  function snapshotSimulationEnd(){for(const a of game.actors){if(network&&a!==game.player&&!(a.isBot&&roomState.isHost))continue;a._simCurr={x:a.x,y:a.y,z:a.z,yaw:a.yaw}}}
  function simulateFixed(dt,now){
    if(!game.player)return;snapshotSimulationStart();game.shotCooldown=Math.max(0,(game.shotCooldown||0)-dt);updatePhase();if(!network&&roomState.phase==='hunt'){for(const h of game.actors)if(h.role==='hider'&&h.alive&&h.prop)h.hiderScore=(h.hiderScore||0)+dt*core.propSurvivalRate(h.prop)}updatePlayer(dt);updateBots(dt);updateNpcs(dt);updateStabilityRecovery(dt);snapshotSimulationEnd();
  }
  function interpolateSimulationRoots(alpha){
    for(const a of game.actors){if(network&&a!==game.player&&!(a.isBot&&roomState.isHost))continue;const p=a._simPrev,c=a._simCurr;if(!p||!c||a.mantle)continue;const t=Math.max(0,Math.min(1,alpha));a.rig.position.set(core.lerp(p.x,c.x,t),core.lerp(p.y,c.y,t),core.lerp(p.z,c.z,t));a.rig.rotation.y=core.lerpAngle(p.yaw,c.yaw,t)}
  }
  function presentFrame(dt,now,alpha){
    if(!game.player)return;game.cinematic?.update(dt);audio?.update(dt);updateRemoteActors(dt);interpolateSimulationRoots(alpha);updateActorVisuals(dt);updateEffects(dt);updateCamera(dt);updateHud();game.w40Truth?.update?.();if(network&&now-game.lastNetworkSend>100){sendNetworkSnapshots();game.lastNetworkSend=now}
  }
  ...
  function applyPerformanceQuality(tier){
    ...
    if(game.world?.group)game.world.group.traverse?.(o=>{if(!o.isMesh)return;if(tier==='low'&&o.userData?.decorativeDetail)o.visible=false;else if(o.userData?.decorativeDetail)o.visible=true});
  }

  function updateQaHud(dt,now){
    const q=game?.qa;if(!q)return;const ms=Math.max(0,dt*1000);q.frameTimes.push(ms);if(q.frameTimes.length>180)q.frameTimes.shift();q.maxMs=Math.max(ms,q.maxMs*.995);q.frames++;q.accum+=dt;if(q.accum>=.5){q.fps=Math.round(q.frames/q.accum);q.frames=0;q.accum=0}if(now-(q.lastHud||0)<250)return;q.lastHud=now;if(q.frameTimes.length){const sorted=[...q.frameTimes].sort((a,b)=>a-b);q.p95Ms=sorted[Math.min(sorted.length-1,Math.floor(sorted.length*.95))]||0}const a=game.player,info=game.renderer.info.render,el=root.querySelector('#ph3Qa');if(!el||!a)return;el.textContent=[
      `QA · ${game.mapKey} · ${q.fps||'--'} fps · p95 ${q.p95Ms.toFixed(1)} ms · peak ${q.maxMs.toFixed(1)} ms`,
      ...
      `sim 60Hz · alpha ${game.frameAlpha.toFixed(2)} · recoveries ${game.stability.recoveries} · ${game.stability.lastReason}`,
      `draw ${info.calls}  tris ${info.triangles}  lines ${info.lines} · quality ${game.qualityTier} · pixel ${game.performance?.pixelRatio?.toFixed?.(2)||'--'}`,
  ...
  function hasLineOfSight(a,b){const q=game.scratch,origin=q.losOrigin.set(a.x,a.y+a.height*.72,a.z),target=q.losTarget.set(b.x,b.y+b.height*.58,b.z),dir=q.losDir.copy(target).sub(origin),dist=dir.length();if(dist<.001)return true;dir.multiplyScalar(1/dist);q.losRay.set(origin,dir);q.losRay.near=.05;q.losRay.far=dist;const hits=q.losRay.intersectObjects(game.world.raycastMeshes,true);return !hits.length||hits[0].distance>=dist-.25;}
  ...
  function initEffectPool(){
    game.fxShared={
      spark:new THREE.SphereGeometry(.025,6,4),beam:new THREE.CylinderGeometry(.014,.022,1,8,1,true),orb:new THREE.SphereGeometry(.055,10,7),ring:new THREE.RingGeometry(.055,.11,20),poof:new THREE.SphereGeometry(.3,12,8),transformRing:new THREE.TorusGeometry(.34,.025,8,24),placementRing:new THREE.RingGeometry(.18,.26,24),flashOrb:new THREE.SphereGeometry(.22,12,8)
    };game.fxPools={spark:[],beam:[],orb:[],ring:[],poof:[],transformRing:[],placementRing:[],flashOrb:[],light:[]};
  }
  function createFxObject(kind){
    if(kind==='light')return new THREE.PointLight(0xffffff,1,2,2);
    const geo=game.fxShared[kind],wire=kind==='flashOrb',mat=new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:1,depthWrite:false,side:['ring','placementRing'].includes(kind)?THREE.DoubleSide:THREE.FrontSide,wireframe:wire}),mesh=new THREE.Mesh(geo,mat);mesh.userData.fxPoolKind=kind;return mesh;
  }
  function acquireFx(kind){const pool=game.fxPools?.[kind];if(!pool)return null;const obj=pool.pop()||createFxObject(kind);obj.visible=true;obj.position.set(0,0,0);obj.scale.set(1,1,1);obj.rotation.set(0,0,0);obj.quaternion.set(0,0,0,1);if(obj.material){obj.material.opacity=1;obj.material.color?.setHex?.(0xffffff)}game.scene.add(obj);return obj}
  function releaseFxObject(kind,obj){if(!obj)return;game.scene.remove(obj);obj.visible=false;const pool=game.fxPools?.[kind];if(pool&&pool.length<48)pool.push(obj);else if(kind!=='light')obj.material?.dispose?.()}
  function releaseEffect(e){if(!e?.mesh)return;const kind=e.poolKind||e.mesh.userData?.fxPoolKind;if(kind){releaseFxObject(kind,e.mesh);return}game.scene.remove(e.mesh);e.mesh.geometry?.dispose?.();if(e.ownsMaterial)e.mesh.material?.dispose?.()}
  function addEffect(e){if(!game||!e)return;while(game.effects.length>=Math.max(12,game.fxBudget||84)){const oldest=game.effects.shift();releaseEffect(oldest)}game.effects.push(e)}
  function disposeEffectPool(){if(!game)return;for(const e of game.effects||[])releaseEffect(e);game.effects.length=0;for(const pool of Object.values(game.fxPools||{}))for(const obj of pool)obj.material?.dispose?.();for(const geo of Object.values(game.fxShared||{}))geo?.dispose?.();game.fxPools=null;game.fxShared=null}

  function updateEffects(dt){
    for(let i=game.effects.length-1;i>=0;i--){const e=game.effects[i];e.life-=dt;if(e.kind==='tracer'&&e.mesh.material)e.mesh.material.opacity=Math.max(0,e.life/e.max);if(e.kind==='ring'){const k=1+(1-e.life/e.max)*(e.grow||1);e.mesh.scale.setScalar(k);if(e.mesh.material)e.mesh.material.opacity=Math.max(0,e.life/e.max)}if(e.kind==='spark'){e.vy-=8*dt;e.mesh.position.x+=e.vx*dt;e.mesh.position.y+=e.vy*dt;e.mesh.position.z+=e.vz*dt;if(e.mesh.material)e.mesh.material.opacity=Math.max(0,e.life/e.max)}if(e.kind==='breakShard'){e.vy-=6.5*dt;e.mesh.position.x+=e.vx*dt;e.mesh.position.y+=e.vy*dt;e.mesh.position.z+=e.vz*dt;e.mesh.rotation.x+=3.2*dt;e.mesh.rotation.z+=2.4*dt}if(e.life<=0){game.effects.splice(i,1);releaseEffect(e)}}game.motionFx?.update(dt);if(game.world?.group)art.animateAmbience(game.world.group,performance.now()*.001,{player:game.player,dt})
  }
  ...
  function updateCamera(dt){
    ...
    if(a===player&&game.cameraRig.state.collapsedFor>.72){game.cameraRig.reset(track,game.world.colliders,{yaw:a.yaw,pitch:(w40pres?.W40_PROP_HUNT_CAMERA?.recoveryPitch??.012),distance:game.cameraRig.cfg.cameraDistance,dog:isDog(a.person),height:a.prop?a.height*.55:(isDog(a.person)?.64:(game.cameraRig?.cfg?.cameraHeight||1.34)),forceSnap:true,reason:'sustained camera collapse recovery'});game.stability.recoveries++;game.stability.lastReason='camera collapse';game.stagingQa?.setRecovery('automatic camera collapse recovery')}
    ...
    game.cameraYaw=Number.isFinite(game.cameraRig.state.yaw)?game.cameraRig.state.yaw:a.yaw;game.cameraPitch=Number.isFinite(game.cameraRig.state.pitch)?game.cameraRig.state.pitch:(w40pres?.W40_PROP_HUNT_CAMERA?.recoveryPitch??.012);game.cameraActualDistance=Number.isFinite(game.cameraRig.state.actualDistance)?game.cameraRig.state.actualDistance:game.cameraRig.cfg.cameraDistance;player.cameraHidden=player.alive&&game.cameraActualDistance<.72&&!player.prop;updateAimAssistIndicator();
  }
  ...
  function muzzleWorldPosition(a,out=game.scratch.shotStart){out.set(a.x,a.y+a.height*.68,a.z);const muzzle=a.rig?.userData?.parts?.weapon?.userData?.muzzle;muzzle?.getWorldPosition?.(out);return out}
  ...
  function shoot(){
    ...
    const q=game.scratch,ray=q.raycaster;ray.setFromCamera(q.screenCenter,game.camera);ray.far=32;const targets=[...game.world.raycastMeshes];for(const d of game.decoys)d.traverse(o=>{if(o.isMesh&&o.visible)targets.push(o)});for(const n of game.world.npcs||[])n.traverse(o=>{if(o.isMesh&&o.visible)targets.push(o)});for(const b of game.actors)if(b!==a&&b.alive)b.rig.traverse(o=>{if(o.isMesh&&o.visible)targets.push(o)});
    const hits=ray.intersectObjects(targets,false).filter(h=>h.object.visible),assist=aimAssistTarget(a),cameraPoint=q.shotPoint.copy(game.camera.position).addScaledVector(ray.ray.direction,25);if(hits.length)cameraPoint.copy(hits[0].point);
    ...
    if(assist?.point)cameraPoint.copy(assist.point);
    const validated=revalidateShotFromMuzzle(a,cameraPoint,targets),hitPoint=validated.hitPoint,target=validated.target;spawnTracer(a,hitPoint);spawnImpactBurst(hitPoint,{actorHit:!!target,object:validated.hit?.object||null});showShotImpact(!!target);
    if(target&&target.role==='hider'){registerHit(a,target);showHit();audio?.oneShot('impact',{volume:.08,pitch:.78+Math.random()*.12,pan:studio.computeStereoPan(game.cameraYaw,a,target)})}else{spawnSparks(hitPoint,6);audio?.oneShot('impact',{volume:.05,pitch:1.08+Math.random()*.18,pan:0})}
  }
  function registerHit(shooter,target){if(roomState.phase!=='hunt'||!target.alive)return;if(network&&ws?.readyState===1){ws.send(JSON.stringify({type:'action',action:'hit',targetId:target.id}));return}target.health--;target.locked=false;target._hitShake=.22;if(target===game.player)showDamage(target.health);gameplay.playTransientAnimation(target,'hit',460);spawnSparks(game.scratch.v3.set(target.x,target.y+target.height*.55,target.z),9);addFeed(`Hit ${target.prop||target.person.name}: ${Math.max(0,target.health)}/3.`);if(target.health<=0){spawnPropBreak(target);playSinCue();if(shooter){shooter.score=shooter.score||{hunterElims:0};shooter.score.hunterElims=(shooter.score.hunterElims||0)+1}if(roomState.settings.mode==='chaos'){target.role='hunter';target.health=3;target.prop=null;target.alive=true;target.disguiseOptions=[];applyActorVisual(target);addFeed(`${target.person.name} joins the hunters.`)}else{target.alive=false;target.rig.visible=false;beginGhostMode(target);addFeed(`${target.person.name} was found.`)}}}
  function spawnTracer(a,end){
    const parts=a.rig.userData.parts,muzzle=parts?.weapon?.userData?.muzzle,start=game.scratch.v1;if(muzzle)muzzle.getWorldPosition(start);else start.set(a.x,a.y+a.height*.68,a.z);const dir=game.scratch.v2.copy(end).sub(start),len=dir.length();if(len>.02){const beam=acquireFx('beam');beam.material.color.setHex(0x8ff4ff);beam.material.opacity=.92;beam.scale.set(1,len,1);beam.position.copy(start).add(end).multiplyScalar(.5);beam.quaternion.setFromUnitVectors(game.scratch.v3.set(0,1,0),dir.multiplyScalar(1/len));addEffect({kind:'tracer',poolKind:'beam',mesh:beam,life:.15,max:.15})}
    const orb=acquireFx('orb');orb.material.color.setHex(0xc8fbff);orb.position.copy(start);addEffect({kind:'tracer',poolKind:'orb',mesh:orb,life:.07,max:.07});const flash=acquireFx('light');flash.color.setHex(0x7eefff);flash.intensity=4.2;flash.distance=2.8;flash.decay=2;flash.position.copy(start);addEffect({kind:'light',poolKind:'light',mesh:flash,life:.055,max:.055});
  }
  function spawnImpactBurst(pos,{actorHit=false,object=null}={}){const color=actorHit?0x8ff4ff:(object?.material?.metalness||0)>.45?0xffe19b:0xffc86a,ring=acquireFx('ring');ring.material.color.setHex(color);ring.material.opacity=.9;ring.position.copy(pos);ring.quaternion.copy(game.camera.quaternion);addEffect({kind:'ring',poolKind:'ring',mesh:ring,life:.24,max:.24,grow:2.2});const light=acquireFx('light');light.color.setHex(color);light.intensity=actorHit?3.2:2.1;light.distance=1.7;light.decay=2;light.position.copy(pos);addEffect({kind:'light',poolKind:'light',mesh:light,life:.08,max:.08});}
  function spawnSparks(pos,n){const room=Math.max(0,(game.fxBudget||84)-game.effects.length),count=Math.min(n,room,game.qualityTier==='low'?4:game.qualityTier==='medium'?7:10);for(let i=0;i<count;i++){const mesh=acquireFx('spark');mesh.material.color.setHex(0xffcf5a);mesh.position.copy(pos);addEffect({kind:'spark',poolKind:'spark',mesh,life:.28+Math.random()*.25,max:.5,vx:(Math.random()-.5)*2.2,vy:1+Math.random()*2,vz:(Math.random()-.5)*2.2})}}
  function spawnPoof(x,y,z){const mesh=acquireFx('poof');mesh.material.color.setHex(0xfff1d1);mesh.material.opacity=.75;mesh.position.set(x,y,z);addEffect({kind:'spark',poolKind:'poof',mesh,life:.55,max:.55,vx:0,vy:.35,vz:0})}

  function showShotImpact(actorHit=false){const c=root.querySelector('#ph3Crosshair');if(!c)return;c.classList.remove('impact','target-hit');void c.offsetWidth;c.classList.add('impact');if(actorHit)c.classList.add('target-hit');setTimeout(()=>c.classList.remove('impact','target-hit'),150)}
  ...
  function updatePropMotionVisual(a,dt,directional,motion){const m=a.propMesh;if(!m)return;const local=directional?.local||{x:0,z:0},moving=(directional?.speed||0)>.12&&!a.locked,base=m.userData.p3BaseScale,hitShake=Math.max(0,a._hitShake||0);if((a._propTransform||0)>0&&base){a._propTransform=Math.max(0,a._propTransform-dt*3.4);const t=1-a._propTransform,e=1-Math.pow(1-t,3),pop=1+Math.sin(Math.PI*t)*.08;m.scale.set(base.x*e*pop,base.y*e*pop,base.z*e*pop)}else if(base){m.scale.set(base.x,base.y,base.z)}m.rotation.z=gameplay.damp(m.rotation.z||0,moving?-local.x*.075:0,9,dt)+(hitShake?Math.sin(performance.now()*.09)*hitShake*.12:0);m.rotation.x=gameplay.damp(m.rotation.x||0,moving?local.z*.035:0,9,dt);m.position.y=gameplay.damp(m.position.y||0,moving?Math.abs(Math.sin((a.animTime||0)*6))*0.018:0,10,dt);a._hitShake=Math.max(0,hitShake-dt);a.rig.rotation.x=gameplay.damp(a.rig.rotation.x||0,0,10,dt);a.rig.rotation.z=gameplay.damp(a.rig.rotation.z||0,0,10,dt);}
  function spawnTransformBurst(x,y,z){spawnPoof(x,y,z);const ring=acquireFx('transformRing');ring.material.color.setHex(0x9fe7c2);ring.material.opacity=.85;ring.position.set(x,y,z);ring.rotation.x=Math.PI/2;addEffect({kind:'ring',poolKind:'transformRing',mesh:ring,life:.5,max:.5,grow:1.45});}
  function spawnPlacementRing(x,y,z){const ring=acquireFx('placementRing');ring.material.color.setHex(0xc7f0a5);ring.material.opacity=.8;ring.position.set(x,y+.012,z);ring.rotation.x=-Math.PI/2;addEffect({kind:'ring',poolKind:'placementRing',mesh:ring,life:.42,max:.42,grow:1.65});}
  function spawnFlashBurst(x,y,z){const orb=acquireFx('flashOrb');orb.material.color.setHex(0xfff5b8);orb.material.opacity=.75;orb.position.set(x,y,z);addEffect({kind:'ring',poolKind:'flashOrb',mesh:orb,life:.55,max:.55,grow:4.6});const light=acquireFx('light');light.color.setHex(0xfff0a4);light.intensity=4.5;light.distance=4.8;light.decay=2;light.position.set(x,y,z);addEffect({kind:'light',poolKind:'light',mesh:light,life:.18,max:.18});}
  ...
  function dropDecoy(){
    const a=game?.player;if(!a||a.role!=='hider'||!a.prop||a.decoys<=0)return;const place=safeDecoyPlacement(a);if(!place){APP.toast('No safe room for a decoy here');return}if(network&&ws?.readyState===1){ws.send(JSON.stringify({type:'action',action:'decoy',position:{x:place.x,y:place.y,z:place.z},rotation:place.rotation}));return}a.decoys--;spawnDecoy(a.prop,place.x,place.y,place.z,{rotation:place.rotation});addFeed(`Decoy placed. ${a.decoys}/10 left.`);
  }
