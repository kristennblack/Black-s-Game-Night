...
(function(){
  ...
  function updatePlayer(dt){
    ...
    const moving=Math.hypot(a.vx,a.vz)>.15;a._moving=moving;if(moving&&!a._wasMoving){a._moveTransition='startMove';a._moveTransitionUntil=performance.now()+260}else if(!moving&&a._wasMoving){a._moveTransition='stopMove';a._moveTransitionUntil=performance.now()+290}moveActor(a,a.vx*dt,a.vz*dt,(a._jumpBuffer||0)>0||a.vy>1.2);resolveActorOverlap(a);input.jumpQueued=false;
  ...
  function handleNetworkAction(m){if(m.action==='flash'){const source=game.actorsById.get(m.playerId);if(source&&game.player.role==='hunter'&&core.dist2(source,game.player)<3.5)flashScreen();addFeed(`${source?.person?.name||'A hider'} fired a flash.`)}if(m.action==='hit'){const target=game.actorsById.get(m.targetId);if(target){target.health=m.health;target.alive=m.alive;target.role=m.role;gameplay.playTransientAnimation(target,'hit',460);if(!target.alive)spawnPoof(target.x,target.y+.6,target.z);addFeed(`${target.person.name} was hit. ${target.health}/3.`)}}if(m.action==='decoy')spawnDecoy(m.prop,m.position?.x||0,m.position?.y||0,m.position?.z||0);}
  ...
  function updateActorVisuals(dt){
    for(const a of game.actors){
      ...
      a.recoil=Math.max(0,(a.recoil||0)-dt*7);a.rig.visible=!(a===game.player&&a.cameraHidden);
      let focus=null;
      ...
      const attention=gameplay.updateAttention(a,dt,focus,{headHeight:isDog(a.person)?.82:1.55,maxDistance:7});
      if(a.authored&&a.hasAuthoredClips&&a.animMixer){
        let base=a.anim||'idle';const aiming=a.role==='hunter'&&(a===game.player?(input.aim||game.padAim):!!a.ai?.detected),transient=a._transientAnim&&Date.now()<(a._transientAnimUntil||0)?a._transientAnim:null;
        if(base==='run'&&a._sprinting)base='sprint';
        const speed=Math.hypot(a.vx||0,a.vz||0),baseScale=base==='sprint'?clamp(speed/5.6,.78,1.28):base==='run'?clamp(speed/4.3,.72,1.3):base==='walk'?clamp(speed/2.35,.72,1.32):1;
        if((aiming||transient==='fire')&&['idle','walk','run','sprint','aim','fire'].includes(base)){
          const lower=['walk','run','sprint'].includes(base)?base:(speed>.18?(a._sprinting?'sprint':'walk'):'idle'),overlay=transient==='fire'?'fire':'aim';
          a.animMixer.playLayered(lower,overlay,{baseTimeScale:baseScale,overlayTimeScale:1,overlayOnce:overlay==='fire'});
        }else if(a._moveTransition&&performance.now()<(a._moveTransitionUntil||0)&&['idle','walk','run','sprint'].includes(base)){
          a.animMixer.play(a._moveTransition,{timeScale:1});
        }else{a._moveTransition=null;a._moveTransitionUntil=0;a.animMixer.play(base,{timeScale:baseScale})}
        a.animMixer.update(dt);a._studioAnimState=a.animMixer.getState?.();
      }else{
        gameplay.animateFamilyRig(a,dt,{aim:a===game.player&&(input.aim||game.padAim),recoil:a.recoil||0,lookPitch:a.pitch||0,turnRate:a.turnRate||0,speed:Math.hypot(a.vx||0,a.vz||0),grounded:a.grounded,attention});
        studio.updateProceduralFace(a,dt,{expression:a.anim==='hit'?'hurt':a.role==='hunter'?'focused':'neutral'});studio.applyFootIK(a,THREE,{heightAt:(x,z)=>core.supportHeight(x,z,a.radius,game.world.colliders,a.y+.1,.5),dt,maxLift:.1});
      }
      if(!a.prop&&a.rig.visible)for(const ev of gameplay.consumeMotionEvents(a)){game.motionFx?.emit(a.x,a.y,a.z,{strength:ev.strength,kind:ev.type});if(a===game.player||core.dist2(a,game.player)<8)audio?.oneShot(ev.type==='land'?'land':'step',{volume:ev.type==='land'?.11:.03,pitch:.9+Math.random()*.18,pan:studio.computeStereoPan(game.cameraYaw,game.player,a),surface:propSurfaceAt(a)})}
    }
  }
  ...
  function updateCamera(dt){
    ...
    game.cameraYaw=game.cameraRig.state.yaw;game.cameraPitch=game.cameraRig.state.pitch;game.cameraActualDistance=game.cameraRig.state.actualDistance;a.cameraHidden=game.cameraActualDistance<.72&&!a.prop;updateAimAssistIndicator();
  }
  ...
  function dropDecoy(){const a=game?.player;if(!a||a.role!=='hider'||!a.prop||a.decoys<=0)return;if(network&&ws?.readyState===1){ws.send(JSON.stringify({type:'action',action:'decoy'}));return}a.decoys--;const supportY=core.supportHeight(a.x,a.z,a.radius,game.world.colliders,a.y+.08,.5);const decoyY=Number.isFinite(supportY)?supportY:a.y;spawnDecoy(a.prop,a.x,decoyY,a.z);addFeed(`Decoy dropped. ${a.decoys}/10 left.`);}
  function spawnDecoy(type,x,y,z){const mesh=createPropMesh(type);mesh.position.set(x+(Math.random()-.5)*.5,Number.isFinite(Number(y))?Number(y):0,z+(Math.random()-.5)*.5);mesh.rotation.y=Math.random()*Math.PI*2;game.scene.add(mesh);game.decoys.push(mesh);spawnPoof(mesh.position.x,mesh.position.y+.28,mesh.position.z);}
  ...
  window.__PROP_HUNT_REAL3D__={version:'GAME-NIGHT-STAGING-PHASE-S-GAMEPLAY-TABLETOP-REALISM-17',renderer:'WebGL',three:'0.185.1',usesDepthBuffer:true,usesCanvas2D:false};
