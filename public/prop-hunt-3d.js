...
(function(){
  ...
  async function upgradePapaProductionSlice(w){
    ...
    try{
      ...
      for(const obj of [environment,propSet]){obj.userData.productionVerticalSlice=true;obj.traverse?.(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;o.userData.productionVerticalSlice=true}});w.group.add(obj)}
  ...
  function shoot(){const a=game?.player;if(!a||a.role!=='hunter'||!a.alive||roomState.phase!=='hunt'||game.shotCooldown>0)return;game.shotCooldown=.11;a.recoil=.7;gameplay.playTransientAnimation(a,'fire',240);audio?.oneShot('zap',{volume:.12,pitch:.92+Math.random()*.12});game.cameraRig?.kick(.028,(Math.random()-.5)*.008,.025);const ray=new THREE.Raycaster();ray.setFromCamera(new THREE.Vector2(0,0),game.camera);ray.far=28;const targets=[...game.world.raycastMeshes];for(const d of game.decoys)d.traverse(o=>{if(o.isMesh&&o.visible)targets.push(o)});for(const n of game.world.npcs||[])n.traverse(o=>{if(o.isMesh&&o.visible)targets.push(o)});for(const b of game.actors)if(b!==a&&b.alive)b.rig.traverse(o=>{if(o.isMesh&&o.visible)targets.push(o)});const hits=ray.intersectObjects(targets,false).filter(h=>h.object.visible);let cameraPoint=game.camera.position.clone().add(ray.ray.direction.clone().multiplyScalar(25));if(hits.length)cameraPoint=hits[0].point.clone();const validated=revalidateShotFromMuzzle(a,cameraPoint,targets),hitPoint=validated.hitPoint,target=validated.target;spawnTracer(a,hitPoint);if(target&&target.role==='hider'){registerHit(a,target);showHit();audio?.oneShot('impact',{volume:.08,pitch:.78+Math.random()*.12,pan:studio.computeStereoPan(game.cameraYaw,a,target)})}else{spawnSparks(hitPoint,5);audio?.oneShot('impact',{volume:.045,pitch:1.08+Math.random()*.18,pan:0})}}
  ...
  window.__PROP_HUNT_REAL3D__={version:'GAME-NIGHT-STAGING-PHASE-K-LAB02-THREE-GAMES-08',renderer:'WebGL',three:'0.185.1',usesDepthBuffer:true,usesCanvas2D:false};
