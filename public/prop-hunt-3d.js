 ...
 * v3.1.1-p3-hunter-release-combat
 *
...
(function(){
  ...
  function startEngine(mapKey){
    ...
    game={renderer,scene,camera,clock,mapKey,world:null,actors:[],actorsById:new Map(),player:null,keys,feed:[],effects:[],decoys:[],cameraYaw:Math.PI,cameraPitch:.18,cameraDistance:3.65,cameraActualDistance:3.65,shotCooldown:0,recoil:0,round:roomState.round||1,lastNetworkSend:0,startedAt:performance.now(),nearProp:null,controlDisposers:[],padPrev:{},padShoot:false,padSprint:false,spectateTarget:null,lastDamageAt:0,lastPhase:roomState.phase,hideReleaseAnnounced:false,qa:{frames:0,accum:0,fps:0,lastHud:0}};
  ...
  function bindControls(){
    ...
    // Crosshair-first combat: there is no separate aim state. Looking moves the reticle and holding fire repeats at a controlled rate.
  ...
  function update(dt,now){
    ...
    game.shotCooldown=Math.max(0,(game.shotCooldown||0)-dt);if((input.shoot||game.padShoot)&&!blindHunter)shoot();game.cinematic?.update(dt);audio?.update(dt);updatePhase();updatePlayer(dt);updateBots(dt);updateRemoteActors(dt);updateActorVisuals(dt);updateNpcs(dt);updateEffects(dt);updateCamera(dt);updateHud();if(network&&now-game.lastNetworkSend>100){sendNetworkSnapshots();game.lastNetworkSend=now}
  }
  ...
  function updateCamera(dt){
    ...
    const aim=false,sprinting=a===player&&gameplay.wantsSprint(keys,input,{sprint:game.padSprint},{strength:Math.min(1,Math.hypot(a.vx,a.vz)/(game.cameraRig.cfg.runSpeed||1))});game.cameraRig.state.aim=false;game.cameraRig.update(a,game.world.colliders,dt,{aim:false,sprinting,dog:isDog(a.person),height:a.prop?a.height*.55:(isDog(a.person)?.64:1.17),velocity:{x:a.vx,z:a.vz},turnRate:a.turnRate||0,cameraBob:(a._motion?.landing||0)*-.032});
  ...
  function updateHud(){
    const a=game.player,phase=root.querySelector('#ph3Phase'),role=root.querySelector('#ph3Role'),health=root.querySelector('#ph3Health'),load=root.querySelector('#ph3Load'),feed=root.querySelector('#ph3Feed'),prompt=root.querySelector('#ph3Prompt'),spectate=root.querySelector('#phSpectate');if(!phase)return;updateHunterHideOverlay();const left=roomState.phaseEndsAt?Math.max(0,roomState.phaseEndsAt-Date.now()):0;
    ...
    load.innerHTML=a.role==='hider'?`Disguise: <b>${esc(a.prop||'none')}</b><br>Changes left: <b>${a.propChanges}</b><br>Decoys: <b>${a.decoys}/10</b><br>Flash: <b>${a.flash?'READY':'USED'}</b><br>Lock: <b>${a.locked?'LOCKED':'FREE'}</b><br>Health carries through disguise changes.`:`Weapon: <b>Prop Zapper</b><br>Crosshair: <b>ALWAYS ACTIVE</b><br>Hold SHOOT: <b>RAPID FIRE</b><br>Shots stop at walls, props and players.<br>Unlimited reserve ammunition.`;
