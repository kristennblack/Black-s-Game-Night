...
export function create3DArtKit(THREE){
  ...
  function buildHumanRig(style={},opts={}){
    ...
    const hair=sphere(.229,hairM,[0,1.005,.014],[.96,.62,.95]);upperBody.add(hair);
    ...
    if(id==='kristen'){
      // Approved Kristen: shoulder-length wavy blonde hair, simple and light enough for mobile.
      addLongBackHair(.52);
    }
    if(id==='vanessa'){
      // Approved Vanessa: long voluminous blonde curls with the maroon top, no hood silhouette.
      addLongBackHair(.62);for(const sx of [-.19,.19])upperBody.add(capsule(.055,.42,hairM,[sx,.74,.12],[.08,0,sx*.12],[.9,1,.86]));
    }
    if(id==='logan'){
    ...
    if(id==='holly'){
      // Approved Holly: bright blonde double buns, cream padded top and blue backpack.
      for(const sx of [-.14,.14]){upperBody.add(sphere(.082,hairM,[sx,1.16,.015],[1,1,.95]));upperBody.add(capsule(.024,.14,hairM,[sx*.78,.99,-.08],[.12,0,sx>0?-.10:.10],[.8,1,.8]));}
    ...
    if(id==='dorothy'){
      // Approved Dorothy: blonde updo, no glasses, blue dress/top and a cream floral apron.
      upperBody.add(sphere(.12,hairM,[0,1.17,.075],[1,.82,.9]));
      const skirt=mesh(new THREE.CylinderGeometry(.34,.47,.72,20,1,true),top,[0,.60,0]);g.add(skirt);
      const apronM=material('fabric',style.apron||0xe8d6b8,{seed:hashString(id)+101});g.add(box(.47,.60,.028,apronM,[0,.69,-.285]));
      const floralM=material('fabric',style.floral||0xd98573,{seed:hashString(id)+103});for(const [xx,yy] of [[-.12,.84],[.10,.70],[-.08,.56],[.13,.47]])g.add(sphere(.025,floralM,[xx,yy,-.304],[1,.65,.35]));
    }
  ...
  function createPropMesh(type,d={}){
    ...
    switch(type){
      ...
      case 'Tractor':{const body=material('paintedMetal',col,{seed:501}),dark=material('rubber',0x222526);add(box(bw*.68,bh*.46,bd*.72,body,[0,bh*.5,0]));add(box(bw*.32,bh*.42,bd*.58,body,[bw*.34,bh*.78,0]));for(const sx of [-bw*.34,bw*.34])for(const zz of [-bd*.38,bd*.38])add(torus(sx<0?bd*.25:bd*.28,bd*.09,dark,[sx,bh*.24,zz],[0,Math.PI/2,0]));add(cylinder(.045,.055,bh*.48,material('metal',0x45484a),[-bw*.08,bh*.94,-bd*.18],[],10));break}
      case 'Motorcycle':{const frame=material('paintedMetal',col,{seed:503});for(const x of [-bw*.36,bw*.36])add(torus(bd*.34,bd*.09,rub,[x,bd*.34,0],[0,Math.PI/2,0]));add(tubeBetween([-bw*.28,bd*.35,0],[bw*.18,bh*.68,0],.045,frame));add(tubeBetween([bw*.18,bh*.68,0],[bw*.38,bd*.35,0],.045,frame));add(box(bw*.42,.08,bd*.32,material('fabric',0x302b29),[-bw*.02,bh*.66,0]));add(cylinder(.08,.1,.22,frame,[bw*.1,bh*.54,0],[0,0,Math.PI/2],12));break}
      case 'Papa Chair':{const yellow=material('fabric',col,{seed:505});add(box(bw*.72,bh*.38,bd*.72,yellow,[0,bh*.36,0]));add(box(bw*.72,bh*.62,bd*.18,yellow,[0,bh*.78,bd*.3],[-.1,0,0]));for(const sx of [-bw*.43,bw*.43])add(box(bw*.16,bh*.55,bd*.72,yellow,[sx,bh*.47,0]));break}
      case 'Tool Chest':{add(box(bw,bh,bd,metal,[0,bh/2,0]));for(let i=0;i<5;i++)add(box(bw*.82,.05,bd*.04,steel,[0,bh*.22+i*bh*.13,-bd*.51]));for(const sx of [-bw*.37,bw*.37])add(cylinder(.055,.055,.07,rub,[sx,.035,-bd*.3],[Math.PI/2,0,0],10));break}
      case 'Air Compressor':{add(cylinder(bd*.32,bd*.32,bw*.62,material('paintedMetal',col),[0,bh*.42,0],[0,0,Math.PI/2],18));add(box(bw*.36,bh*.32,bd*.5,metal,[bw*.2,bh*.67,0]));for(const sx of [-bw*.32,bw*.32])add(torus(.12,.045,rub,[sx,.15,0],[0,Math.PI/2,0]));break}
      case 'Barrel Stack':{for(let y=0;y<2;y++)for(let x=-1;x<=1;x++){const yy=.43+y*.78,xx=x*bw*.31+(y?bw*.15:0);add(cylinder(bd*.24,bd*.24,.72,material('paintedMetal',shadeHex(col,x*.04+y*.05)),[xx,yy,0],[],16));}break}
      case 'Cabin Bed':{const pine=material('wood',0x8a603c,{seed:601}),darkPine=material('wood',0x553624,{seed:602}),blanket=material('fabric',0x405a43,{seed:603}),cream=material('fabric',0xe8d8bd,{seed:604});add(box(bw,.2,bd,pine,[0,.28,0]));for(const sx of [-bw*.43,bw*.43])for(const sz of [-bd*.42,bd*.42])add(box(.08,.54,.08,darkPine,[sx,.27,sz]));add(box(bw*.9,.19,bd*.84,cream,[0,.49,0]));add(box(bw*.88,.06,bd*.46,blanket,[0,.61,bd*.15]));add(box(bw*.95,bh*.58,.1,darkPine,[0,bh*.66,bd*.47]));for(const xx of [-bw*.25,bw*.25])add(box(bw*.28,.11,bd*.2,cream,[xx,.67,-bd*.25]));break}
      case 'Cabin TV':{const frame=material('paintedWood',0x553624,{seed:611}),screen=new THREE.MeshStandardMaterial({color:0x182329,emissive:0x274b55,emissiveIntensity:.42,roughness:.13});add(box(bw,bh*.74,bd*.38,frame,[0,bh*.55,0]));add(box(bw*.89,bh*.61,.025,screen,[0,bh*.56,-bd*.21]));add(box(bw*.43,.07,bd*.62,frame,[0,.08,0]));add(tubeBetween([0,.09,0],[0,bh*.24,0],.04,material('metal',0x444a49)));break}
      case 'Cabin Dresser':{const pine=material('wood',0x6e4931,{seed:621}),trim=material('wood',0x8a603c,{seed:622}),brass=material('metal',0xb49352);add(box(bw,bh,bd,pine,[0,bh/2,0]));for(let i=0;i<4;i++){const y=.19+i*(bh*.2);add(box(bw*.84,bh*.16,.025,trim,[0,y,-bd*.51]));for(const xx of [-bw*.16,bw*.16])add(sphere(.035,brass,[xx,y,-bd*.55]));}add(box(bw*1.04,.07,bd*1.08,trim,[0,bh+.035,0]));break}
      case 'Cabin Lamp':{const iron=material('metal',0x444a49),shade=material('fabric',0xe8d8bd,{seed:631});add(cylinder(.035,.045,bh*.68,iron,[0,bh*.38,0],[],12));add(cylinder(bw*.32,bw*.38,.08,material('paintedMetal',0x553624),[0,.04,0],[],18));const cone=mesh(new THREE.ConeGeometry(bw*.45,bh*.31,22,1,true),shade,[0,bh*.82,0]);add(cone);add(sphere(.07,new THREE.MeshStandardMaterial({color:0xffe6ad,emissive:0xffc978,emissiveIntensity:1.25}),[0,bh*.75,0]));break}
      case 'Cabin Rug':{const base=material('fabric',0x7d3d38,{seed:641}),stripe=material('fabric',0xe8d8bd,{seed:642});add(box(bw,.025,bd,base,[0,.013,0]));for(let i=-3;i<=3;i++)add(box(bw*.92,.008,bd*.035,stripe,[0,.032,i*bd*.12]));break}
      case 'Tree':{const trunk=material('wood',0x684936,{seed:509}),leaf=material('foliage',col,{seed:510});add(cylinder(bw*.16,bw*.22,bh*.55,trunk,[0,bh*.28,0],[],12));for(const [x,y,z,s] of [[0,.64,0,.48],[-.22,.74,.04,.42],[.22,.77,.02,.42],[0,.88,-.12,.38]])add(sphere(bw*s,leaf,[x*bw,y*bh,z*bw],[1,.86,1]));break}
