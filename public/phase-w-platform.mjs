...
// Arcade reward contract: +5 Arcade Tokens on first play of each game; +10 Arcade Tokens for the daily 3-different-games challenge.
const GAMES={
...
function trackPlay(p,id){const s=readJSON(LOCAL_KEY,{tokens:0,achievements:{},plays:{},records:{}}),legacy=readJSON('bfgn_arcade_achievements_v1',{}),nowAt=Date.now(),today=new Date().toISOString().slice(0,10);s.achievements={...(legacy||{}),...(s.achievements||{})};s.plays=s.plays||{};s.plays[id]=(s.plays[id]||0)+1;let firstDelta=0,dailyDelta=0;if(s.plays[id]===1){firstDelta=5;s.tokens=(s.tokens||0)+firstDelta;s.achievements[`first:${id}`]={label:`First play: ${cfg.name}`,at:nowAt}}if(s.daily?.date!==today)s.daily={date:today,games:{},rewarded:false};s.daily.games=s.daily.games||{};s.daily.games[id]=true;const dailyCount=Object.keys(s.daily.games).length;if(dailyCount>=3&&!s.daily.rewarded){dailyDelta=10;s.daily.rewarded=true;s.tokens=(s.tokens||0)+dailyDelta;s.achievements[`daily:${today}`]={label:'Daily challenge: 3 different arcade games',at:nowAt}}localStorage.setItem(LOCAL_KEY,JSON.stringify(s));if(firstDelta||dailyDelta)window.addEventListener('load',()=>setTimeout(()=>window.BFGNReaction?.(`+${firstDelta+dailyDelta} Arcade Tokens${dailyDelta?' · Daily Challenge':''}`,'Achievement'),500),{once:true});fetch('/api/arcade/record',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({profileId:p.profileId,name:p.name,gameId:id,type:'play',tokenDelta:firstDelta,achievement:firstDelta?`first:${id}`:null,label:`First play: ${cfg.name}`})}).catch(()=>{});if(dailyDelta)fetch('/api/arcade/record',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({profileId:p.profileId,name:p.name,gameId:id,type:'reward',tokenDelta:dailyDelta,achievement:`daily:${today}`,label:'Daily challenge: 3 different arcade games'})}).catch(()=>{})}
async function syncServer(p,id){const r=await fetch(`/api/arcade/profile?profileId=${encodeURIComponent(p.profileId)}`,{cache:'no-store'});if(!r.ok)return;const d=await r.json();if(!d.profile)return;const s=readJSON(LOCAL_KEY,{tokens:0,achievements:{},plays:{}});s.tokens=Number(d.profile.tokens||0);s.achievements={...(d.profile.achievements||{}),...(s.achievements||{})};localStorage.setItem(LOCAL_KEY,JSON.stringify(s))}
