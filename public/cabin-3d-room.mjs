...
export function mountCabinRoom3D(container,{room,catalogById,isOwner=false,selectedId=null,onSelect=()=>{},onMove=()=>{},wallItem=null,floorItem=null}={}){
  ...
  const configureRoot=(root,q,item,surface,pos)=>{root.name=`cabin-item-${q.itemId}`;root.userData.placementId=q.id;root.userData.surface=surface;root.userData.itemId=q.itemId;root.userData.placementRef=q;root.userData.w25Spec=W25_HOME_PRODUCTION[q.itemId]||null;root.position.set(pos.x,pos.y,pos.z);root.rotation.y=THREE.MathUtils.degToRad(Number(q.rotation)||0);if(surface==='wall'){root.rotation.x=0;root.position.z=-ROOM_D/2+.18;const box=new THREE.Box3().setFromObject(root),h=box.max.y-box.min.y;root.position.y=Math.max(.18,pos.y-h*.1)}return root};
  const loadProductionModel=(root,q,item,surface)=>{const spec=W25_HOME_PRODUCTION[q.itemId];if(!spec?.model)return;w25Loader().then(loader=>loader.loadAsync(spec.model)).then(gltf=>{if(!root.parent)return;const old=[...root.children];for(const child of old){root.remove(child);disposeTree(child)}const model=gltf.scene||gltf.scenes?.[0];if(!model)throw new Error(`No scene in ${spec.model}`);model.name=`w25-model-${spec.sku}`;model.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;o.userData.placementId=q.id;o.userData.surface=surface;pickables.push(o)}});root.add(model);root.userData.productionModelLoaded=true;if(spec.interaction==='toggle_light'){const light=new THREE.PointLight(0xffcf8a,11,3.6,2);light.position.set(0,.72,0);light.castShadow=true;root.add(light);const initialOn=q.state?.lampOn!==false;light.visible=initialOn;w25LampLights.set(q.id,{light,on:initialOn});root.userData.lampOn=initialOn}if(spec.interaction==='sit'){root.userData.seatTarget={x:0,y:.73,z:-.02,rotationY:Math.PI};}showSelection(selectedId)}).catch(err=>{root.userData.productionModelError=String(err?.message||err);root.userData.productionModelLoaded=false;console.warn('W25 production model fallback in cabin',q.itemId,err)})};
  for(const q of room?.placements||[]){const item=catalogById?.[q.itemId];if(!item)continue;const pos=placementWorld(q),surface=q.surface||'floor',spec=W25_HOME_PRODUCTION[q.itemId];let root;if(spec?.model){root=new THREE.Group();const fallback=art.createCatalogHomeMesh(item);fallback.name='w25-loading-fallback';fallback.traverse(o=>{if(o.material){o.material=o.material.clone();o.material.transparent=true;o.material.opacity=.24}});root.add(fallback);configureRoot(root,q,item,surface,pos);registerPickables(root,q,surface);scene.add(root);placementRoots.push(root);loadProductionModel(root,q,item,surface)}else{root=art.createCatalogHomeMesh(item);const scale=categoryScale(item);root.scale.setScalar(scale);configureRoot(root,q,item,surface,pos);registerPickables(root,q,surface);scene.add(root);placementRoots.push(root)}}
  let helper=null;
  ...
  return {setSelected(id){selectedId=id;showSelection(id)},interact(id){const root=placementRoots.find(x=>x.userData.placementId===id),spec=root?.userData?.w25Spec;if(!root||!spec)return {ok:false,type:'none',message:'This item has no W25 interaction.'};if(spec.interaction==='toggle_light'){const rec=w25LampLights.get(id);if(!rec)return {ok:false,type:'toggle_light',message:'The lamp model is still loading.'};rec.on=!rec.on;rec.light.visible=rec.on;root.userData.lampOn=rec.on;const q=root.userData.placementRef;if(q){q.state={...(q.state||{}),lampOn:rec.on}}root.traverse(o=>{const m=o.material;if(m?.emissive&&/bulb/i.test(o.name||''))m.emissiveIntensity=rec.on?1.5:0.03});return {ok:true,type:'toggle_light',on:rec.on,message:`Lamp turned ${rec.on?'on':'off'}.`}}if(spec.interaction==='sit')return {ok:false,type:'sit',seatTarget:root.userData.seatTarget||null,message:'Seat target is authored. Final sit animation waits for the approved production avatar mesh.'};if(spec.interaction==='surface')return {ok:true,type:'surface',message:'This table is a valid decor surface.'};return {ok:true,type:spec.interaction||'place_rotate',message:'Production interaction hook is ready.'}},dispose(){cancelAnimationFrame(raf);ro.disconnect();renderer.dispose();disposeTree(scene);container.replaceChildren()}};
}
