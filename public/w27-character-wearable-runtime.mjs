...
export const W27_JOHN_REPAIRED_FITS=Object.freeze({
  cowboyHat:Object.freeze({
    id:'W25-A01',model:'/models/w25/w25-dark-brown-ranch-cowboy-hat.glb',parent:'head',
    position:[0,.155,-.005],rotation:[0,0,0],scale:.38,label:'Dark Brown Ranch Cowboy Hat'
  }),
  aviators:Object.freeze({
    id:'W25-A02',model:'/models/w25/w25-gold-brown-aviators.glb',parent:'head',
    position:[0,.025,-.202],rotation:[0,0,0],scale:.21,label:'Gold / Brown Aviators'
  })
});
...
export async function loadJohnWithW27Wearables({hat=true,aviators=true}={}){
  ...
  for(const [key,enabled] of [['cowboyHat',hat],['aviators',aviators]]){
    if(!enabled)continue;
    const fit=W27_JOHN_REPAIRED_FITS[key],gltf=await loader.loadAsync(fit.model),obj=gltf.scene||gltf.scenes?.[0];
    if(!obj)throw new Error(`${fit.label} GLB contains no scene.`);
    obj.name=`W27_${key}`;obj.position.fromArray(fit.position);obj.rotation.set(...fit.rotation);obj.scale.setScalar(fit.scale);
...
export function mountJohnHeadFitProof(container,{initial='both'}={}){
  if(!container)throw new Error('W27 repaired-head fit proof container required.');
  container.replaceChildren();
  const shell=document.createElement('div');shell.className='w27-fit-shell';
  const status=document.createElement('div');status.className='w27-fit-status';status.textContent='Loading repaired John head + real W25 wearable GLBs…';
  ...
  const setMode=mode=>{if(!attachments)return;attachments.cowboyHat&&(attachments.cowboyHat.visible=mode==='hat'||mode==='both');attachments.aviators&&(attachments.aviators.visible=mode==='aviators'||mode==='both');status.textContent=`W27 REPAIRED HEAD CANDIDATE · ${mode.toUpperCase()} · real GLBs attached to John head bone · DEVICE APPROVAL PENDING`;status.dataset.mode=mode};
  Promise.all([loadJohnWithW27Wearables({hat:true,aviators:true})]).then(([loaded])=>{
