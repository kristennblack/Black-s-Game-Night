...
if(exists('PHASE_R_PROP_HUNT_P2_GAMMON_UX_REPORT.md')&&exists('PHONE_QA_PHASE_R_PROP_HUNT_P2_GAMMON_UX_16.md')){const report=await text('PHASE_R_PROP_HUNT_P2_GAMMON_UX_REPORT.md');report.includes('Prop Hunt P2 + Gammon UX')&&report.includes('screen-first Gammon surface')?pass('Phase R report and real-device QA are preserved'):fail('Phase R preserved report is incomplete');}else fail('Phase R preserved report or QA checklist missing');
...
prop3d.includes('targetYaw=aiming?game.cameraYaw:movingIntent?Math.atan2(intent.directionX,-intent.directionZ):a.yaw')&&prop3d.includes("const aiming=a.role==='hunter'")&&prop3d.includes('id=\"phAim\"')?pass('Phase T/T1 actor-facing lineage preserved under W30 explicit Aim'):fail('Phase T/T1 actor-facing contract incomplete');
...
prop3d.includes('HUNTER_FIRE_INTERVAL=1/4.8')&&prop3d.includes('(input.shoot||game.padShoot)')&&prop3d.includes('bindHoldButton(shootBtn')&&prop3d.includes('bindHoldButton(aim')&&prop3d.includes('id=\"phAim\"')?pass('Phase T1 rapid-fire lineage upgraded to W30 hold Aim + Shoot controls'):fail('Phase T1/W30 hunter combat control contract incomplete');
...
// Phase W30 P0 focused gates: current input, movement, stability and combat contract.
gameplay3d.includes('export function bindHoldButton')&&gameplay3d.includes('lostpointercapture')&&gameplay3d.includes('visibilitychange')&&gameplay3d.includes('blur')?pass('W30 P0 lifecycle-safe touch/hold input manager present'):fail('W30 P0 input lifecycle contract incomplete');
gameplay3d.includes('export function movementSpeedForIntent')&&gameplay3d.includes('export function gaitForMovement')&&gameplay3d.includes('walkSpeed:2.35')&&gameplay3d.includes('jogSpeed:3.35')&&gameplay3d.includes('runSpeed:4.50')&&gameplay3d.includes('sprintSpeed:5.70')&&!prop3d.includes('correctedJoy={x:joy.x,z:-joy.z}')?pass('W30 P0 analog gait tiers present without double joystick inversion'):fail('W30 P0 movement/gait contract incomplete');
prop3d.includes('id=\"phAim\"')&&prop3d.includes('bindHoldButton(aim')&&prop3d.includes('input.aim||game.padAim')&&prop3d.includes("e.pointerType!=='mouse'")?pass('W30 P0 explicit Aim and touch-safe Shoot controls present'):fail('W30 P0 hunter input contract incomplete');
propCore.includes('export function groundSupport')&&propCore.includes('export function mantleTarget')&&prop3d.includes('groundSupport(')&&prop3d.includes('mantleTarget(')?pass('W30 P0 multi-probe grounding and validated mantle present'):fail('W30 P0 grounding/mantle contract incomplete');
prop3d.includes('function alignProp')&&prop3d.includes('id=\"phAlign\"')&&prop3d.includes('a.locked=false')&&prop3d.includes("APP.toast('Prop aligned and locked')")?pass('W30 P0 prop Align and movement-unlock behavior present'):fail('W30 P0 prop lock/align contract incomplete');
prop3d.includes('Math.hypot(safe.x-a.x,safe.z-a.z)>.24')&&prop3d.includes("APP.toast('Not enough room to disguise here')")?pass('W30 P0 disguise clearance rejects large relocation'):fail('W30 P0 disguise placement contract incomplete');
gameplay3d.includes('export function sanitizeActorKinematics')&&prop3d.includes('sanitizeActorKinematics')&&gameplay3d.includes('Number.isFinite')?pass('W30 P0 finite-value recovery guards present'):fail('W30 P0 finite recovery contract incomplete');
prop3d.includes('game.cameraRig.cfg')&&prop3d.includes('input.aim||game.padAim')&&gameplay3d.includes('enabled=()=>true')?pass('W30 P0 camera and protected-hide input gating present'):fail('W30 P0 camera/input gating contract incomplete');

if(exists('MASTER_PROP_HUNT_P0_GAMEPLAY_ENGINE_REBUILD.md')){const directive=await text('MASTER_PROP_HUNT_P0_GAMEPLAY_ENGINE_REBUILD.md');directive.includes('Input and controls')&&directive.includes('REAL-DEVICE GAMEPLAY GATE')?pass('W30 P0 governing gameplay-engine contract packaged'):fail('W30 P0 governing contract incomplete');}else fail('W30 P0 governing contract missing');
if(exists('PHONE_QA_PHASE_W30_PROP_HUNT_P0_GAMEPLAY_54.md')){const qa=await text('PHONE_QA_PHASE_W30_PROP_HUNT_P0_GAMEPLAY_54.md');qa.includes('Mobile input lifecycle')&&qa.includes('P0 DEVICE GATE APPROVED')?pass('W30 P0 phone/desktop Gate 2 checklist packaged'):fail('W30 P0 Gate 2 checklist incomplete');}else fail('W30 P0 Gate 2 checklist missing');
