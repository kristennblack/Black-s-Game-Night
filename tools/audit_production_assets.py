...
checks=[
 ('characters','john',['JohnRig','JohnBody','hips','upperBody','head','leftShoulder','leftElbow','leftHand','rightShoulder','rightElbow','rightHand','leftHip','leftKnee','leftFoot','rightHip','rightKnee','rightFoot','rightHandSocket']),
...
for cat,key,required in checks:
    ...
    if key in ('john','gunner'):
        # John W27 intentionally replaced the legacy photo material with the approved stylized turnaround texture.
        expected_mat='John_ApprovedStylizedFace' if key=='john' else 'Gunner_FacePhoto'
