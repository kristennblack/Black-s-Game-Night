...
const ARCADE_COSMETICS={
 'camp-cap':{slot:'hat',price:10},'cowboy-hat':{slot:'hat',price:20},'birthday-crown':{slot:'hat',price:30},'winter-toque':{slot:'hat',price:25},'flower-crown':{slot:'hat',price:35},
 'round-glasses':{slot:'glasses',price:10},'sunglasses':{slot:'glasses',price:20},'heart-glasses':{slot:'glasses',price:25},'safety-glasses':{slot:'glasses',price:20},
 'red-bandana':{slot:'accessory',price:15},'gold-chain':{slot:'accessory',price:25},'headphones':{slot:'accessory',price:30},'flower-pin':{slot:'accessory',price:15},'rock-charm':{slot:'accessory',price:20}
};
const normalizeEquippedCosmetics=raw=>{const out={hat:null,glasses:null,accessory:null};for(const slot of Object.keys(out)){const id=String(raw?.[slot]||'');if(id&&ARCADE_COSMETICS[id]?.slot===slot)out[slot]=id}return out};
...
export class GameHub {
  ...
  async getArcadeProfile(profileId){const id=String(profileId||'').trim().slice(0,80);if(!id)return null;const p=await this.ctx.storage.get(`arcade-profile:${id}`);return p&&p.profileId?{...p,equippedCosmetics:normalizeEquippedCosmetics(p.equippedCosmetics)}:{profileId:id,name:'Family Player',tokens:0,achievements:{},plays:{},records:{},cosmetics:{},equippedCosmetics:{hat:null,glasses:null,accessory:null},updatedAt:0}}
  ...
  async updateArcadeCosmetic(b){
    ...
    if(action==='buy'){if(!item)throw new Error('Unknown cosmetic');if(!p.cosmetics[itemId]){if(Number(p.tokens||0)<item.price)throw new Error(`Need ${item.price} Arcade Tokens`);p.tokens=Number(p.tokens||0)-item.price;p.cosmetics[itemId]={unlockedAt:now(),slot:item.slot}}}
    ...
    else if(action==='unequip'){const slot=String(b.slot||'');if(!['hat','glasses','accessory'].includes(slot))throw new Error('Unknown cosmetic slot');p.equippedCosmetics[slot]=null}
