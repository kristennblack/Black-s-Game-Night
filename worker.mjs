...
import { DOROTHY_LOOK_BY_ID, STARTER_DOROTHY_LOOK_ID, normalizeDorothyLooks } from './public/dorothy-looks-catalog.mjs';
...
async function handleApi(request){
  ...
  if(u.pathname==='/api/arcade/look'&&request.method==='POST'){const b=await parseBody(request);try{return jsonResponse({ok:true,profile:await activeHub.updateFamilyLook(b)})}catch(err){return jsonResponse({error:err.message},400)}}
...
export class GameHub {
  ...
  async getArcadeProfile(profileId){
    const id=String(profileId||'').trim().slice(0,80);if(!id)return null;
    const p=await this.ctx.storage.get(`arcade-profile:${id}`);
    if(p&&p.profileId){
      const johnLooks=normalizeJohnLooks(p.johnLooks),hollyLooks=normalizeHollyLooks(p.hollyLooks),gunnerLooks=normalizeGunnerLooks(p.gunnerLooks),dorothyLooks=normalizeDorothyLooks(p.dorothyLooks),mollyLooks=normalizeMollyLooks(p.mollyLooks),kelsiLooks=normalizeKelsiLooks(p.kelsiLooks);
      ...
      const equippedDorothyLook=DOROTHY_LOOK_BY_ID[p.equippedDorothyLook]?p.equippedDorothyLook:STARTER_DOROTHY_LOOK_ID;
      ...
      dorothyLooks[equippedDorothyLook]=dorothyLooks[equippedDorothyLook]||{unlockedAt:0,source:'equipped-migration'};
  ...
  async updateFamilyLook(b){
    ...
    const inferred=itemId.startsWith('holly-look-')?'holly':itemId.startsWith('gunner-look-')?'gunner':itemId.startsWith('dorothy-look-')?'dorothy':itemId.startsWith('molly-look-')?'molly':itemId.startsWith('kelsi-look-')?'kelsi':'john';
    ...
    if(action==='buy'){
      ...
      if(!b.rewardKey)throw new Error('Missing reward key');p[sys.ownedKey][itemId]=p[sys.ownedKey][itemId]||{unlockedAt:now(),source:'reward',rewardKey:String(b.rewardKey).slice(0,120)};
    }else throw new Error('Unknown look action');
    p.updatedAt=now();await this.ctx.storage.put(`arcade-profile:${id}`,p);return p;
  }
  async updateJohnLook(b){return this.updateFamilyLook({...b,character:b.character||'john'})}
  ...
  async getCabinRoom(roomKey){
    ...
    if(stored&&Number(stored.decorVersion||0)<19){const migrated={...stored,wallpaper:'bare-pine-wall',flooring:'bare-pine-floor',placements:[],decorVersion:21,updatedAt:now()};await this.ctx.storage.put(`cabin-room:${key}`,migrated);return migrated}
    if(stored&&Number(stored.decorVersion||0)<21){const migrated={...stored,decorVersion:21,updatedAt:now()};await this.ctx.storage.put(`cabin-room:${key}`,migrated);return migrated}
    return stored||{roomKey:key,ownerProfileId:null,ownerName:'',ownerAvatar:key.startsWith('guest:')?'cowboy':key,wallpaper:'bare-pine-wall',flooring:'bare-pine-floor',placements:[],guestbook:[],reactions:[],decorVersion:21,updatedAt:0};
  }
