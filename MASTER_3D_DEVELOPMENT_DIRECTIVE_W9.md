...
# PHASE W.9 PROP HUNT CHARACTER DETAIL + CONTROL OVERRIDE

For Family Prop Hunt, `MASTER_PHASE_W9_PROP_HUNT_CHARACTER_DETAIL_CONTROLS_DIRECTIVE.md` is the governing implementation-quality specification for character geometry, rigging, movement, camera, hunter/hider controls and actual-device acceptance.

This W.9 section has higher precedence than older language in this file that can be read as permission to redesign the family toward generic "stylized realism." The target is instead:

> **the exact approved ultra-simplified family identity, rebuilt with better three-dimensional construction and animation quality.**

That means:
- more rounded/dimensional faces without changing the approved face;
- sculpted hair masses that preserve the approved silhouette;
- better shoulder, arm, wrist and hand construction;
- correct palm/thumb orientation;
- more readable clothing thickness, folds, shoes and materials;
- higher Prop Hunt close-camera LOD0 detail where phone performance allows;
- a shared rig with right trigger-hand grip and left support-hand IK;
- close shoulder camera, centered shot ray, visible zapper/tracer/impact;
- responsive camera-relative analog movement with jump buffer, coyote time and validated auto-mantle;
- role-specific hunter and hider mobile controls that support multi-touch;
- actual-phone visual and control QA before propagation to the remaining family cast.

Do not call a model improved merely because it contains more polygons. If the higher-detail model resembles the approved person less, it is a regression.
