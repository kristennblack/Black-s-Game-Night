import { assignRoles, sanitizeSnapshot, canServerRegisterHit, clamp, PAPA_DISGUISE_POOL, assignDisguiseOptions, propSurvivalRate, roundSeed, weatherForSeed, layoutVariantForSeed, disguisePoolForMap } from './public/prop-hunt-core.mjs';
...
const scoreDefaults=()=>({hiderWins:0,hunterWins:0,hunterElims:0,hiderPoints:0});
const lifetimeDefaults=()=>({rounds:0,hiderPoints:0,hunterElims:0,hiderSurvivals:0,hiderWins:0,hunterWins:0});
const normalizedScore=s=>({...scoreDefaults(),...(s||{})});
const normalizedLifetime=s=>({...lifetimeDefaults(),...(s||{})});

function persistentPlayer(p){
  return {id:p.id,token:p.token,name:p.name,avatar:p.avatar,seat:p.seat,isBot:!!p.isBot,difficulty:p.difficulty||null,ready:!!p.ready,connected:!!p.connected,role:p.role||null,health:p.health??3,alive:p.alive!==false,prop:p.prop||null,propChanges:p.propChanges??3,decoys:p.decoys??10,flash:p.flash!==false,disguiseOptions:[...(p.disguiseOptions||[])],hiderScore:Number(p.hiderScore)||0,disguiseStartedAt:p.disguiseStartedAt||0,eliminatedAt:p.eliminatedAt||0,score:normalizedScore(p.score),lifetime:normalizedLifetime(p.lifetime)};
}
function publicPlayer(p,{maskHide=false,hideOptions=false}={}){
  return {id:p.id,name:p.name,avatar:p.avatar,seat:p.seat,isBot:!!p.isBot,difficulty:p.isBot?p.difficulty:null,ready:!!p.ready,connected:!!p.connected,role:p.role||null,health:p.health??3,alive:p.alive!==false,prop:maskHide?null:(p.prop||null),propChanges:p.propChanges??3,decoys:p.decoys??10,flash:p.flash!==false,disguiseOptions:(maskHide||hideOptions)?[]:[...(p.disguiseOptions||[])],hiderScore:maskHide?0:(Number(p.hiderScore)||0),score:normalizedScore(p.score),lifetime:normalizedLifetime(p.lifetime),live:maskHide?null:(p.live||null)};
}
...
export class PropHuntRoom {
  ...
  stateFor(tokenValue){
    ...
    return {id:this.room.id,createdAt:this.room.createdAt,revision:this.room.revision||0,phase:this.room.phase,phaseEndsAt:this.room.phaseEndsAt||0,round:this.room.round||0,activeMap:this.room.activeMap||mapForRound(this.room.settings?.mapKey||'papa',this.room.round||1),roundSeed:this.room.roundSeed||0,layoutVariant:this.room.layoutVariant||0,weatherPreset:this.room.weatherPreset||'clear',wins:this.room.wins||{hiders:0,hunters:0},settings:this.room.settings,players:this.room.players.map(p=>publicPlayer(p,{maskHide:hideFromHunter&&p.role==='hider',hideOptions:viewer?.id!==p.id})),decoyObjects:hideFromHunter?[]:(this.room.decoyObjects||[]),viewerId:viewer?.id||null,isHost:!!viewer&&viewer.id===this.room.hostPlayerId,hostPlayerId:this.room.hostPlayerId,roundResult:this.room.roundResult||null,roundSummary:this.room.roundSummary||null};
  }
  ...
  accrueHider(p,now=Date.now()){
    if(!p||p.role!=='hider'||!p.alive||!p.prop||!p.disguiseStartedAt)return;
    const elapsed=Math.max(0,(now-p.disguiseStartedAt)/1000);if(elapsed<=0)return;
    const points=elapsed*propSurvivalRate(p.prop);p.hiderScore=(Number(p.hiderScore)||0)+points;p.score=normalizedScore(p.score);p.score.hiderPoints+=points;p.lifetime=normalizedLifetime(p.lifetime);p.lifetime.hiderPoints+=points;p.disguiseStartedAt=now;
  }
  accrueAllHiders(now=Date.now()){for(const p of this.room?.players||[])this.accrueHider(p,now)}

  startRound(){
    const r=this.room,roles=assignRoles(r.players,r.round),seed=roundSeed(r.id,r.round,r.createdAt);r.activeMap=mapForRound(r.settings.mapKey,r.round);const assigned=assignDisguiseOptions(r.players,seed,disguisePoolForMap(r.activeMap),4);
    r.phase='hide';r.phaseEndsAt=Date.now()+Math.max(15,Number(r.settings.hideSeconds)||30)*1000;r.roundResult=null;r.roundSummary=null;r.decoyObjects=[];r.roundSeed=seed;r.layoutVariant=layoutVariantForSeed(seed);r.weatherPreset=weatherForSeed(seed);
    for(const p of r.players){p.role=roles[p.id];p.health=3;p.alive=true;p.prop=null;p.propChanges=3;p.decoys=10;p.flash=true;p.live=null;p.hiderScore=0;p.disguiseStartedAt=0;p.eliminatedAt=0;p.disguiseOptions=p.role==='hider'?[...(assigned[p.id]||[])]:[];p.score=normalizedScore(p.score);p.lifetime=normalizedLifetime(p.lifetime);p.score.hunterElims=0;p.score.hiderPoints=0}
    this.broadcastState(true);
  }
  finishRound(winner){
    const r=this.room;if(r.phase==='roundEnd'||r.phase==='matchEnd')return;const now=Date.now();this.accrueAllHiders(now);
    r.wins[winner]=(r.wins[winner]||0)+1;r.roundResult=winner;
    for(const p of r.players){p.score=normalizedScore(p.score);p.lifetime=normalizedLifetime(p.lifetime);p.lifetime.rounds++;if(p.role===(winner==='hiders'?'hider':'hunter')){p.score[winner==='hiders'?'hiderWins':'hunterWins']++;p.lifetime[winner==='hiders'?'hiderWins':'hunterWins']++}if(winner==='hiders'&&p.role==='hider'&&p.alive)p.lifetime.hiderSurvivals++}
    const hiders=r.players.filter(p=>p.role==='hider').sort((a,b)=>(b.hiderScore||0)-(a.hiderScore||0));const hunters=r.players.filter(p=>p.role==='hunter').sort((a,b)=>(b.score?.hunterElims||0)-(a.score?.hunterElims||0));
    r.roundSummary={winner,weatherPreset:r.weatherPreset,layoutVariant:r.layoutVariant,bestHider:hiders[0]?{id:hiders[0].id,name:hiders[0].name,avatar:hiders[0].avatar,points:Math.round(hiders[0].hiderScore||0),survived:!!hiders[0].alive}:null,bestHunter:hunters[0]?{id:hunters[0].id,name:hunters[0].name,avatar:hunters[0].avatar,eliminations:hunters[0].score?.hunterElims||0}:null};
    if(r.round>=r.settings.rounds){r.phase='matchEnd';r.phaseEndsAt=0}else{r.phase='roundEnd';r.phaseEndsAt=Date.now()+10000}
    this.broadcastState(true);
  }
  advanceClock(){
    const r=this.room;if(!r)return false;const now=Date.now();let changed=false;
    if(r.phase==='hide'&&now>=r.phaseEndsAt){r.phase='hunt';r.phaseEndsAt=now+Math.max(180,Number(r.settings.roundSeconds)||300)*1000;for(const p of r.players)if(p.role==='hider'&&p.alive&&p.prop)p.disguiseStartedAt=now;changed=true}
    if(r.phase==='hunt'){
      this.accrueAllHiders(now);const hiders=r.players.filter(p=>p.role==='hider'&&p.alive);
  ...
  async create(request,url){
    ...
    this.room={id:roomId,createdAt:Date.now(),revision:1,hostToken,hostPlayerId:playerId,phase:'lobby',phaseEndsAt:0,round:0,roundResult:null,wins:{hiders:0,hunters:0},settings:{mode:'classic',mapKey:'papa',rounds:6,hideSeconds:30,roundSeconds:300},decoyObjects:[],players:[{id:playerId,token:playerToken,name:safeName(body.name||'Host'),avatar:safeAvatar(body.avatar||'john'),seat:0,isBot:false,difficulty:null,ready:false,connected:true,role:null,health:3,alive:true,prop:null,propChanges:3,decoys:10,flash:true,score:scoreDefaults(),lifetime:lifetimeDefaults(),disguiseOptions:[],hiderScore:0,disguiseStartedAt:0,eliminatedAt:0,live:null}]};
  ...
  async configure(request){
    ...
    if(MODES.has(b.mode))this.room.settings.mode=b.mode;if(MAP_KEYS.has(b.mapKey))this.room.settings.mapKey=b.mapKey;const rounds=Number(b.rounds);if(Number.isInteger(rounds)&&rounds>=1&&rounds<=12)this.room.settings.rounds=rounds;const hideSeconds=Number(b.hideSeconds),roundSeconds=Number(b.roundSeconds);if(Number.isFinite(hideSeconds)&&hideSeconds>=15&&hideSeconds<=60)this.room.settings.hideSeconds=Math.round(hideSeconds);if(Number.isFinite(roundSeconds)&&roundSeconds>=180&&roundSeconds<=600)this.room.settings.roundSeconds=Math.round(roundSeconds);this.broadcastState(true);return json({ok:true});
  }
  ...
  async rematch(request){
    if(!this.room)return json({error:'Room not found'},404);const b=await request.json().catch(()=>({}));if(!hostOk(this.room,b.hostToken))return json({error:'Host only'},403);if(this.room.phase!=='matchEnd')return json({error:'Match is not complete'},409);this.room.round=1;this.room.wins={hiders:0,hunters:0};this.room.roundResult=null;for(const p of this.room.players){p.health=3;p.alive=true;p.prop=null;p.propChanges=3;p.decoys=10;p.flash=true;p.live=null;p.score=scoreDefaults()}this.startRound();return json({ok:true});
  }
  ...
  async webSocketMessage(ws,message){
    ...
    if(action==='disguise'&&sender.role==='hider'&&sender.alive){const prop=String(msg.prop||'').slice(0,48);if(!prop||!(sender.disguiseOptions||[]).includes(prop))return;if(sender.prop&&sender.propChanges<=0)return;this.accrueHider(sender,Date.now());if(sender.prop)sender.propChanges--;sender.prop=prop;sender.flash=true;sender.disguiseStartedAt=this.room.phase==='hunt'?Date.now():0;this.broadcastState(true);this.broadcastGameplay({type:'action',action:'disguise',playerId:sender.id,prop},null,{hideFromHuntersDuringHide:true});return}
    if(action==='decoy'&&sender.role==='hider'&&sender.alive&&sender.prop&&sender.decoys>0){const req=msg.position||null,live=sender.live||null,finite=req&&[req.x,req.y,req.z].every(Number.isFinite),close=finite&&live&&Math.hypot(req.x-live.x,req.y-live.y,req.z-live.z)<=1.9,position=close?{x:req.x,y:req.y,z:req.z}:(live?{x:live.x,y:live.y,z:live.z}:null),rotation=Number.isFinite(msg.rotation)?msg.rotation:(live?.yaw||0);sender.decoys--;const decoy={id:crypto.randomUUID(),playerId:sender.id,prop:sender.prop,position,rotation};this.room.decoyObjects=this.room.decoyObjects||[];this.room.decoyObjects.push(decoy);this.broadcastState(true);this.broadcastGameplay({type:'action',action:'decoy',playerId:sender.id,prop:sender.prop,position:decoy.position,rotation:decoy.rotation,decoyId:decoy.id},null,{hideFromHuntersDuringHide:true});return}
    ...
    if(action==='hit'&&this.room.phase==='hunt'&&sender.role==='hunter'&&sender.alive){const target=this.room.players.find(p=>p.id===msg.targetId);if(!canServerRegisterHit(sender,target,32))return;target.health=clamp((target.health??3)-1,0,3);let eliminated=false;if(target.health<=0){this.accrueHider(target,Date.now());eliminated=true;sender.score=normalizedScore(sender.score);sender.lifetime=normalizedLifetime(sender.lifetime);sender.score.hunterElims++;sender.lifetime.hunterElims++;target.eliminatedAt=Date.now();if(this.room.settings.mode==='chaos'){target.role='hunter';target.prop=null;target.health=3;target.alive=true;target.disguiseOptions=[];target.disguiseStartedAt=0}else target.alive=false}this.broadcastState(true);this.broadcast({type:'action',action:'hit',playerId:sender.id,targetId:target.id,health:target.health,alive:target.alive,role:target.role,eliminated});this.advanceClock();return}
  }
