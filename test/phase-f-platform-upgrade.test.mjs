...
test('Phase F platform build ID and cache namespace are explicit and fresh',async()=>{
  ...
  assert.match(app,/GAME-NIGHT-STAGING-PHASE-H-TABLETOP-06/);assert.match(sw,/black-family-game-night-staging-phase-h-tabletop-06/);assert.match(qa,/GAME-NIGHT-STAGING-PHASE-H-TABLETOP-06/);assert.equal(version.trim(),'GAME-NIGHT-STAGING-PHASE-H-TABLETOP-06');
});
