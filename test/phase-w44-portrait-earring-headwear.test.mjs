...
test('W44 exact portrait headwear uses semantic head seat and real asset override',()=>{
 ...
 assert.equal(fit.exactCalibration,true);assert.match(fit.anchorMode,/(?:exact-head|w45-visual-head)-helmet-seat/);assert.ok(fit.w>30);assert.match(anchors.portraitAccessoryAsset(item),/^\/cosmetics\/generated\/(?:w44-headwear-3d|w46-approved-headwear)\/firefighter-helmet\.png$/);
});
...
test('W44 headwear renderer uses bottom-seat pivot rather than center-only placement',()=>{
 ...
 assert.match(html,/(?:w44-headwear-3d|w46-approved-headwear)\/birthday-crown\.png/);assert.match(html,/(?:exact-head|w45-visual-head)-crown-seat/);assert.match(html,/--cty:-92%/);
});
...
test('W44 candidate cache is distinct from W43',()=>{
 const s=fs.readFileSync(path.join(root,'public/sw.js'),'utf8');assert.match(s,/PHASE_W44_CACHE/);assert.match(s,/black-family-game-night-staging-candidate-w44-earring-headwear-62/);assert.match(s,/RUNTIME_CACHE=PHASE_W4[56]_CACHE/);
});
