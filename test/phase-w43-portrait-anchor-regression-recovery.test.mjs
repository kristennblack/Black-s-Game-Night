...
test('W43 family arcade uses the selected portrait variant and no longer styles cosmetic images as avatar bases',()=>{
 const js=read('public/phase-w-platform.mjs');assert.match(js,/variant:Number\(p\.variant\)\|\|0/);assert.match(js,/class="avatar-base"/);assert.match(js,/assetFor\(cfg\.person,p\)/);assert.match(js,/W45-HEADWEAR-SCALE-RECOVERY-63/);
});

test('W43 candidate uses a candidate-specific service-worker cache so W30 cached cosmetics cannot reappear',()=>{
 const sw=read('public/sw.js'),app=read('public/app.js');assert.match(sw,/PHASE_W43_CACHE='black-family-game-night-staging-candidate-w43-portrait-anchor-recovery-61'/);assert.match(sw,/PHASE_W44_CACHE='black-family-game-night-staging-candidate-w44-earring-headwear-62'/);assert.match(sw,/const CACHE=PHASE_W30_CACHE/);assert.match(sw,/const RUNTIME_CACHE=PHASE_W45_CACHE/);assert.match(app,/sw\.js\?v=W45-HEADWEAR-SCALE-RECOVERY-63/);assert.match(sw,/w43-portrait-anchor-runtime-qa\.html/);
...
test('W43 shop warns rather than silently rendering an uncalibrated or baked-eyewear portrait',()=>{
 const store=read('public/tokens-store.html');assert.match(store,/portraitGlassesFit/);assert.match(store,/W43 intentionally blocks a second pair of glasses/);assert.match(store,/has not been landmark-calibrated yet/);assert.match(store,/W45-HEADWEAR-SCALE-RECOVERY-63/);
});
