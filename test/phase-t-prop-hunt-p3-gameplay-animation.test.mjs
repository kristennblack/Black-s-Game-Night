...
test('directional aiming classifies forward, backpedal and strafes in facing space',()=>{
  ...
  assert.equal(resolveDirectionalLocomotion(forward,{aiming:true}).semantic,'jog');
...
test('player faces travel direction as hider and camera/crosshair direction as hunter',()=>{
  ...
  assert.match(src,/resolveDirectionalLocomotion\(a,\{aiming,sprinting:sprint,gait:a\._gait\}\)/);
});
...
test('jump landing and mantle have a stronger gameplay-feel pass',()=>{
  const src=read('public/prop-hunt-3d.js');
  for(const token of ['_landingStrength','_hardLandTimer','duration:.38','const lift=clamp(m.t/.62','push=clamp((m.t-.12)/.88,0,1)' ]) assert.ok(src.includes(token),token);
});
...
test('hider disguise, decoy and flash actions have deliberate readable feedback',()=>{
  ...
  assert.match(src,/safeDecoyPlacement/);assert.match(src,/core\.groundSupport/);
});
