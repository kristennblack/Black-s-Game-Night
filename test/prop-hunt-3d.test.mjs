...
test('movement blocks solid walls and jump input can auto-mantle a reasonable ledge',()=>{
  ...
  const crate={x:0,z:-.7,y:0,w:1.2,d:1.1,h:.85,solid:true,climbable:true,walkableTop:true};
...
test('mobile/desktop controls expose hold Aim/Shoot plus hider Prop, Flash, Decoy, Lock and Align',()=>{
  for(const id of ['phAim','phShoot','phJump','phSprint','phProp','phFlashBtn','phDecoy','phLock','phAlign','phJoy'])assert.ok(js.includes(id),id);
  assert.match(js,/bindHoldButton\(aim/);assert.match(js,/bindHoldButton\(shootBtn/);
