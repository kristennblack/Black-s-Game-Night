...
test('W20 release is current while W18 and W19 remain explicit historical milestones',()=>{
...
test('W20 universal fitting keeps every wearable visible on every family avatar and migrates legacy slot saves',()=>{
  ...
  assert.equal(eq.face,'round-glasses');assert.equal(eq.neck,'gold-chain');assert.equal(eq.hat,'camp-cap');
...
test('W20 cabin preserves W19 rooms, retains bare-shell migration for older rooms and supports owned architectural finishes',async()=>{
  ...
  let room=await hub.getCabinRoom('kristen');assert.equal(room.decorVersion,20);assert.equal(room.placements.length,1);assert.equal(room.placements[0].id,'kept');
  ...
  room=await hub.getCabinRoom('john');assert.equal(room.decorVersion,20);assert.deepEqual(room.placements,[]);assert.equal(room.wallpaper,'bare-pine-wall');
