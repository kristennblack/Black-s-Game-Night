...
test('W8 release identity remains preserved while W22 is current',()=>{
 assert.equal(read('CURRENT_RELEASE.txt').trim(),'GAME-NIGHT-STAGING-PHASE-W23-CATALOG-FIT-ACCESSORIES-45');
 const pkg=JSON.parse(read('package.json')),app=read('public/app.js'),sw=read('public/sw.js');
 assert.equal(pkg.version,'3.21.0-staging-phase-w23-catalog-fit-accessories-45');
 ...
 assert.match(app,/sw\.js\?v=W22-CATALOG-APPROVAL-STUDIO-44/);
