...
test('W8 release identity remains preserved while W22 is current',()=>{
 assert.equal(read('CURRENT_RELEASE.txt').trim(),'GAME-NIGHT-STAGING-PHASE-W22-CATALOG-APPROVAL-STUDIO-44');
 const pkg=JSON.parse(read('package.json')),app=read('public/app.js'),sw=read('public/sw.js');
 assert.equal(pkg.version,'3.20.0-staging-phase-w22-catalog-approval-studio-44');
