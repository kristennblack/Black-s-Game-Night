...
test('W8 cosmetic foundation remains compatible while W20 expands it into universal layered slots',()=>{
 for(const slot of ['hat','face','filter','earrings','neck','top','outerwear','bottom','shoes','attachment'])assert.ok(COSMETIC_SLOTS.includes(slot),slot);
 assert.equal(COSMETIC_CATALOG.length,2000);
 assert.ok(COSMETIC_CATALOG.filter(x=>x.slot==='hat').length>=100);assert.ok((COSMETIC_CATALOG.filter(x=>x.slot==='face').length+COSMETIC_CATALOG.filter(x=>x.slot==='glasses').length)>=100);assert.ok(COSMETIC_CATALOG.filter(x=>x.slot==='attachment').length>=90);
 const eq=normalizeEquipped({hat:'camp-cap'});assert.equal(eq.hat,'camp-cap');assert.equal(eq.face,null);assert.equal(eq.attachment,null);
 const store=read('public/tokens-store.html');assert.match(store,/Game Night Tokens/i);assert.match(store,/Master Catalog/i);assert.match(store,/cosmeticOverlayHTML/);assert.match(store,/collection/i);
});
...
test('W8 release identity remains preserved while W22 is current',()=>{
 assert.equal(read('CURRENT_RELEASE.txt').trim(),'GAME-NIGHT-STAGING-PHASE-W23-CABIN-REGRESSION-RECOVERY-48');
 const pkg=JSON.parse(read('package.json')),app=read('public/app.js'),sw=read('public/sw.js');
 assert.equal(pkg.version,'3.21.3-staging-phase-w23-cabin-regression-recovery-48');
 ...
 assert.match(app,/sw\.js\?v=W23-CABIN-REGRESSION-RECOVERY-48/);
