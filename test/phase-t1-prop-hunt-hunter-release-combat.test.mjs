...
test('hunter gets an opaque hide countdown and explicit hold-to-aim control',()=>{const js=read('public/prop-hunt-3d.js'),css=read('public/prop-hunt-3d.css');for(const token of ['phHideBlind','HIDERS ARE HIDING','phHideCountdown','isHunterHidePhase','updateHunterHideOverlay','HUNT!','id="phAim"','bindHoldButton(aim'])assert.ok(js.includes(token),token);assert.match(css,/\.ph3d-hide-blind\{[^}]*background:#000/);assert.match(css,/z-index:60/)});

test('protected hide phase freezes hunter movement, look and weapon',()=>{const js=read('public/prop-hunt-3d.js');assert.match(js,/if\(isHunterHidePhase\(\)\)\{a\.vx=a\.vz=a\.vy=0/);assert.match(js,/if\(!blindHunter\)gameplay\.applyGamepadLook/);assert.match(js,/roomState\.phase!=='hunt'/);assert.match(js,/input\.shoot=false;input\.aim=false;input\.sprint=false/);assert.match(js,/game\.padShoot=false;game\.padAim=false;game\.padSprint=false/)});

test('hunter has explicit Aim plus controlled hip-fire/hold Shoot',()=>{const js=read('public/prop-hunt-3d.js');assert.match(js,/const HUNTER_FIRE_INTERVAL=1\/4\.8/);assert.match(js,/input\.aim\|\|game\.padAim/);assert.match(js,/game\.shotCooldown=HUNTER_FIRE_INTERVAL/);assert.match(js,/if\(\(input\.shoot\|\|game\.padShoot\)&&!blindHunter\)shoot\(\)/);assert.match(js,/bindHoldButton\(aim/);assert.match(js,/bindHoldButton\(shootBtn/);assert.match(js,/SHOOT supports mild hip-fire/)});
