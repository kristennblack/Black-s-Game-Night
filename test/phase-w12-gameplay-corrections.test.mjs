...
test('W12 Prop Hunt increases speed, corrects mobile direction, and places the weapon rig in front',async()=>{
 const {prop,shared}=await files();
 assert.match(shared,/walkSpeed:3\.15,runSpeed:5\.35/);
 assert.match(prop,/correctedJoy=\{x:joy\.x,z:-joy\.z\}/);
