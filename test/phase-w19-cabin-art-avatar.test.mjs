...
const W22='GAME-NIGHT-STAGING-PHASE-W30-PROP-HUNT-P0-GAMEPLAY-54';
...
test('W19 and W21 identities remain historical while W22 is current',()=>{
  ...
  assert.equal(JSON.parse(read('package.json')).version,'3.27.0-staging-phase-w30-prop-hunt-p0-gameplay-54');
...
test('W19 room editing supports direct selection, tap-to-move, rotate, store, wall-floor snapping and blueprint duplication',()=>{
  ...
  for(const token of ['mountCabinRoom3D','onSelect','onMove','data-rotate','data-duplicate','data-delete','data-surface','snapPlacement','validateCabinPlacement'])assert.ok(js.includes(token),token);
...
test('W19 server persists bare room defaults and placement surfaces while preserving owner-only blueprint rules',async()=>{
  ...
  const itemId=STARTER_CABIN_BLUEPRINT_IDS[0];const saved=await h.updateCabinRoom({profileId:'k',name:'Kristen',avatar:'kristen',roomKey:'kristen',action:'save',wallpaper:'bare-pine-wall',flooring:'bare-pine-floor',placements:[{id:'a',itemId,x:3,z:4,rotation:90,surface:'floor'}]});assert.equal(saved.placements[0].surface,'floor');assert.equal(saved.decorVersion,21);
  await assert.rejects(()=>h.updateCabinRoom({profileId:'k',name:'Kristen',avatar:'kristen',roomKey:'kristen',action:'save',wallpaper:'bare-pine-wall',flooring:'bare-pine-floor',placements:[{id:'a',itemId,x:3,z:4,rotation:90,surface:'wall'}]}),/cannot be placed on the wall/);
});
