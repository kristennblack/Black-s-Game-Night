...
test('W19 release identity is current and W18 remains preserved historically',()=>{
  ...
  assert.equal(JSON.parse(read('package.json')).version,'3.17.0-staging-phase-w19-cabin-art-avatar-41');
  ...
  assert.match(app,/CURRENT_BUILD=PHASE_W19_RELEASE/);assert.match(sw,/const CACHE=PHASE_W19_CACHE/);
});
...
test('W19 cabin starts as a bare pine architectural shell with a tiny low-end starter crate',()=>{
  ...
  assert.match(js,/CABIN_DECOR_VERSION=19/);assert.match(js,/wallpaper:'bare-pine-wall'/);assert.match(js,/flooring:'bare-pine-floor'/);assert.match(js,/placements:\[\]/);
  assert.match(js,/decorVersion\|\|0\)<CABIN_DECOR_VERSION/);assert.match(js,/placements:\[\]/);assert.match(css,/generated\/empty-room-shell\.svg/);
});

test('all 400 cabin records have their own browse and placeable artwork identities',()=>{
  assert.equal(CABIN_ROOM_ITEM_CATALOG.length,400);
  ...
  assert.equal(thumbs.size,400);assert.equal(places.size,400);assert.equal(thumbHashes.size,400);assert.equal(placeHashes.size,400);
...
test('W19 universal avatar fit never hides a cosmetic, including on Kelsi, Molly and Gunner',()=>{
  ...
  assert.match(cosmetics,/no avatar\/variant may silently hide a cosmetic/);assert.match(styles,/aspect-ratio:4\/5/);assert.match(store,/aspect-ratio:4\/5/);
});
...
test('W19 automatically migrates any persisted legacy room to the empty shell without touching ownership',async()=>{
  ...
  const h=new GameHub({storage,waitUntil(){}},{}),r=await h.getCabinRoom('kristen');assert.equal(r.ownerProfileId,'k');assert.deepEqual(r.placements,[]);assert.equal(r.wallpaper,'bare-pine-wall');assert.equal(r.flooring,'bare-pine-floor');assert.equal(r.decorVersion,19);assert.equal(r.guestbook.length,1);
});
test('W19 server persists bare room defaults and placement surfaces while preserving owner-only blueprint rules',async()=>{
  const h=hub(),d=await h.getCabinRoom('kristen');assert.equal(d.wallpaper,'bare-pine-wall');assert.equal(d.flooring,'bare-pine-floor');assert.equal(d.decorVersion,19);assert.deepEqual(d.placements,[]);
  const itemId=STARTER_CABIN_BLUEPRINT_IDS[0];const saved=await h.updateCabinRoom({profileId:'k',name:'Kristen',avatar:'kristen',roomKey:'kristen',action:'save',wallpaper:'bare-pine-wall',flooring:'bare-pine-floor',placements:[{id:'a',itemId,x:3,z:4,rotation:90,surface:'wall'}]});assert.equal(saved.placements[0].surface,'wall');assert.equal(saved.decorVersion,19);
});
...
test('W19 Family Mystery, Prop Hunt, Island Life and Molly share the cabin art language in priority order',()=>{
  ...
  assert.match(prop,/W19 shared cabin collection art/);assert.match(prop,/Cabin Dresser/);assert.match(prop,/Cabin TV/);
  assert.match(island,/RUSTIC_CABIN_3D_PALETTE/);assert.match(island,/createPropMesh\('Cabin Bed'/);
