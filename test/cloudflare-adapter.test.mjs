...
test('Launch assets advertise the Phase E staging build and use a fresh service-worker cache', async()=>{
  ...
  assert.match(appSource,/APP_VERSION='GAME-NIGHT-STAGING-PHASE-T1-PROP-HUNT-HUNTER-RELEASE-COMBAT-19'/);
  assert.match(swSource,/black-family-game-night-staging-phase-t1-prop-hunt-hunter-release-combat-19/);
});
...
test('John Black defaults to Birthday Legend and supports all 30 complete John looks', async()=>{
  const {hub}=makeHub();
  const created=await post(hub,'create',{name:'John Black',gameType:GAME_TYPES.SCREW});
  ...
  assert.equal(john.variant,2,'John should start on Birthday Legend look #3');
  await post(hub,'profile',{roomId:created.roomId,playerToken:created.playerToken,name:'John Black',avatar:'john',variant:29,outfitVariant:0,color:john.color});
  s=await state(hub,created.roomId,created.playerToken);
  john=s.players.find(p=>p.id===s.viewerId);
  assert.equal(john.variant,29,'John should be able to select look #30');
});

test('All 30 John look image files are packaged and the UI exposes the Looks Shop collection', async()=>{
  const {access,readFile}=await import('node:fs/promises');
  for(let i=1;i<=30;i++) await access(new URL(`../public/avatars/styles/john-look-${String(i).padStart(2,'0')}.jpg`,import.meta.url));
  const appSource=await readFile(new URL('../public/app.js',import.meta.url),'utf8');
  const cssSource=await readFile(new URL('../public/styles.css',import.meta.url),'utf8');
  assert.match(appSource,/JOHN_LOOK_CATALOG\.map/);
  assert.match(appSource,/30 complete Looks Shop outfits/);
  assert.match(appSource,/john-looks-shop\.html/);
