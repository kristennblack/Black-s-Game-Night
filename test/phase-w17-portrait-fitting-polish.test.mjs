...
test('W17 historical release remains preserved while W20 is current and Cabin route remains live',()=>{
 ...
 const app=read('public/app.js');assert.match(app,/PHASE_W17_RELEASE='GAME-NIGHT-STAGING-PHASE-W17-CABIN-COSMETICS-POLISH-39'/);assert.match(app,/CURRENT_BUILD=PHASE_W30_RELEASE/);assert.match(app,/href="\/cabin\.html">Visit the Cabin<\/a>/);
 const sw=read('public/sw.js');assert.match(sw,/PHASE_W17_CACHE/);assert.match(sw,/const CACHE=PHASE_W30_CACHE/);
});
