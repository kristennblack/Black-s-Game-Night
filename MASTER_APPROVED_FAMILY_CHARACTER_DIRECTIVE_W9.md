...
## 12. PHASE W.9 PROP HUNT CLOSE-CAMERA DETAIL OVERRIDE

Phase W.9 increases geometric/material fidelity for close third-person Prop Hunt while preserving every W.5 identity lock.

For approved human hero characters in Prop Hunt:
- LOD0 may target roughly 8,000 to 12,000 triangles when the extra budget is spent on face/head silhouette, hair, shoulders/arms/hands, major clothing volume and shoes;
- medium/distant LODs must reduce aggressively so phone performance remains protected;
- face construction should include rounded cheek/jaw/nose/lid/mouth volume sufficient for subtle facial animation;
- hair must preserve the exact turnaround silhouette using layered sculpted masses;
- clothing gains only major readable volume/folds, while small seams/patterns remain texture-led;
- hands must have correct thumb/palm orientation and cannot use negative-scale mirroring that produces backwards wrists;
- hunter weapon grip is right trigger hand plus left support-hand IK with explicit muzzle/aim sockets;
- the approved base character must remain recognisable with all store cosmetics removed.

The detailed per-character Prop Hunt production cards and control/rig acceptance tests are governed by `MASTER_PHASE_W9_PROP_HUNT_CHARACTER_DETAIL_CONTROLS_DIRECTIVE.md`.

If a higher-detail model drifts from the approved turnaround, the increased detail is a regression, not an improvement.
